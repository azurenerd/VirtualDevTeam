...
public class FrameworkAdapterTests : IDisposable
{
    ...
    private FrameworkInvocation NewFrameworkInvocation(string frameworkId = "baseline") => new()
    {
        ...
        {
            JudgeFeedback = "Improve error handling",
            InitialScores = new Dictionary<string, int> { ["ac"] = 6, ["design"] = 7 },
            OriginalPatch = "--- a/file.cs\n+++ b/file.cs\n",
            RubberDuckFeedback = "Consider edge cases",
        },
    };
    ...
    public void BaselineAdapter_exposes_correct_identity()
    {
        ...
        Assert.Equal(Timeout.InfiniteTimeSpan, adapter.DefaultTimeout);
    ...
    public void BaselineAdapter_MapToStrategy_preserves_all_fields()
    {
        ...
        Assert.NotNull(si.Revision);
        Assert.Equal(fw.Revision!.JudgeFeedback, si.Revision!.JudgeFeedback);
        Assert.Equal(fw.Revision.InitialScores, si.Revision.InitialScores);
        Assert.Equal(fw.Revision.OriginalPatch, si.Revision.OriginalPatch);
    }
    ...
    private sealed class StubMcpLocator : VirtualDevTeam.Core.Mcp.IMcpServerLocator
    {
        public VirtualDevTeam.Core.Mcp.McpServerLaunchSpec Resolve()
