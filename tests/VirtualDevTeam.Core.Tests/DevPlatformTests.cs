...
public class DevPlatformTests
{
    ...
    public void DevPlatformConfig_DefaultsToGitHub()
    {
        ...
        Assert.Equal(DevPlatformAuthMethod.GhCli, config.AuthMethod);
    ...
    public void DevPlatformAuthMethod_HasAllExpectedValues()
    {
        ...
        Assert.Contains(DevPlatformAuthMethod.GhCli, values);
    }

    [Fact]
    public void DevPlatformConfig_GhCliAuthMethod_CanBeSet()
    {
        var config = new DevPlatformConfig
        {
            Platform = DevPlatformType.GitHub,
            AuthMethod = DevPlatformAuthMethod.GhCli
        };
    ...
    // ──────────────────── ConfigValidator — GhCli ────────────────────

    [Fact]
    public void ConfigValidator_GhCli_DoesNotRequireGitHubToken()
    {
        var config = new VirtualDevTeamConfig();
        config.Project.Name = "TestProject";
        config.Project.GitHubRepo = "owner/repo";
        config.Project.GitHubToken = ""; // Empty — GhCli provides token dynamically
        config.Project.DefaultBranch = "main";
        config.DevPlatform.AuthMethod = DevPlatformAuthMethod.GhCli;

        // Add a minimal model to avoid unrelated validation errors
        ...
        config.Agents.ProgramManager.ModelTier = "standard";
        config.Agents.Researcher.ModelTier = "standard";
        config.Agents.Architect.ModelTier = "standard";
        config.Agents.SoftwareEngineer.ModelTier = "standard";
        config.Agents.TestEngineer.ModelTier = "standard";

        var errors = ConfigValidator.Validate(config);

        Assert.DoesNotContain(errors, e => e.Contains("GitHubToken"));
    }

    [Fact]
    public void ConfigValidator_Pat_RequiresGitHubToken()
    {
        var config = new VirtualDevTeamConfig();
        config.Project.Name = "TestProject";
        config.Project.GitHubRepo = "owner/repo";
        config.Project.GitHubToken = ""; // Empty — should fail for PAT
        config.Project.DefaultBranch = "main";
