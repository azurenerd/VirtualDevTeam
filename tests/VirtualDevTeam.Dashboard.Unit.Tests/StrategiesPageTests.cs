...
public sealed class StrategiesPageTests : TestContext
{
        ...
        EnabledStrategiesInfo? enabled = null)
    {
        ...
        Services.AddSingleton<IPlatformLinkService>(new TestPlatformLinkService());
...
/// <summary>
/// Test stub for <see cref="IPlatformLinkService"/> used by Razor component tests.
/// Mirrors the production behaviour of <c>PlatformLinkService</c>: returns internal
/// <c>/repository/...</c> routes so tests don't need to wire <c>VirtualDevTeamConfig</c>.
/// </summary>
internal sealed class TestPlatformLinkService : IPlatformLinkService
{
    public bool InternalNavigationDefault => true;
    public string PlatformDisplayName => "GitHub";
    public string BuildPullRequestUrl(int prNumber, string? platformUrl = null) => $"/repository/pull-request/{prNumber}";
    public string BuildIssueUrl(int issueNumber, string? platformUrl = null) => $"/repository/issue/{issueNumber}";
    public string BuildFileUrl(string path, string? branch = null, string? platformUrl = null) => $"/repository/files/{(path ?? string.Empty).TrimStart('/')}";
    public bool IsInternal(string? url) => !string.IsNullOrWhiteSpace(url) && url!.StartsWith("/", StringComparison.Ordinal);
