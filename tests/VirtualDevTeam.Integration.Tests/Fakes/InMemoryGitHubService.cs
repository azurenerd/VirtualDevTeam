namespace VirtualDevTeam.Integration.Tests.Fakes;
...
public sealed class InMemoryGitHubService : IGitHubService
{
    ...
    /// <summary>Dump all file trees for diagnostic purposes.</summary>
    public string DumpFiles()
    {
        lock (_lock)
        {
            var sb = new System.Text.StringBuilder();
            foreach (var (branch, tree) in _files.OrderBy(kv => kv.Key))
            {
                sb.AppendLine($"Branch: {branch} ({tree.Count} files)");
                foreach (var (path, content) in tree.OrderBy(kv => kv.Key))
                    sb.AppendLine($"  {path} ({content.Length} chars)");
            }
            return sb.ToString();
        }
    }
    ...
    /// <summary>
    /// In-memory stand-in for the release-asset upload path. Returns a deterministic
    /// fake URL that matches the shape of a real release-asset download URL so
    /// consumers parsing it (e.g. the image proxy URL-translator) behave identically
    /// in tests.
    /// </summary>
    public Task<string?> UploadImageAsReleaseAssetAsync(
        string filename, byte[] content, string contentType = "image/png", CancellationToken ct = default)
    {
        lock (_lock)
        {
            _binaryFiles[$"release-asset:{filename}"] = content.ToArray();
            return Task.FromResult<string?>(
                $"https://github.com/{RepositoryFullName}/releases/download/screenshots-store/{filename}");
        }
    }
