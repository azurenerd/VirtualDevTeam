...
- ❌ **NEVER** start `scripts/start-runner.ps1` from Windows PowerShell 5.1. A configured CLI wrapper can freeze there without ever spawning `copilot.exe`. ✅ **ALWAYS** launch the runner from PowerShell 7+ (`pwsh`). The start script now enforces this with a version guard.
...
76. **Stateful CLI wrapper cross-contamination in parallel instances**: A CLI wrapper that persists CWD state across sessions can cause two parallel VDT instances to cross-contaminate (Checkers instance references Tetris scratch dir). Workaround: only one instance can use such a wrapper; others must set `"wrapperCommand": ""`.
