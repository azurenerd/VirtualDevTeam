...
public sealed class ConfigurationService : IConfigurationService
{
    ...
    private VirtualDevTeamConfig? _lastSavedConfig;

    private readonly RunCoordinator _runCoordinator;
    private readonly AgentStateStore _stateStore;
    private readonly IGateCheckService? _gateCheckService;
    private readonly IWebHostEnvironment _env;
    private readonly VirtualDevTeam.Core.AI.IImageGenerationService? _imageGenService;
    ...
    public ConfigurationService(
        ...
        IGateCheckService? gateCheckService = null,
        VirtualDevTeam.Core.AI.IImageGenerationService? imageGenService = null,
        DevelopSettingsService? developSettingsService = null)
    {
        _config = config;
        ...
        _developSettingsService = developSettingsService;
    ...
    public VirtualDevTeamConfig GetCurrentConfig() => _lastSavedConfig ?? _config.CurrentValue;
    ...
    public async Task<PatValidationResult> ValidatePatAsync(string token, string repoFullName, CancellationToken ct = default)
    {
        ...
        try
        {
            var client = new Octokit.GitHubClient(new Octokit.ProductHeaderValue("VirtualDevTeam-Validate"))
            {
        ...
        if (_imageGenService is null)
        {
            return new VirtualDevTeam.Core.AI.ImageValidationReport
            {
                ...
                AuthCheck = new VirtualDevTeam.Core.AI.ValidationCheck
                {
                    ...
                    Passed = false,
                    Detail = "IImageGenerationService is not registered.",
                },
            };
        }

        if (request.Settings is not null && _developSettingsService is not null)
        {
            try
            {
                var settings = await _developSettingsService.LoadAsync(ct);
                settings.AzureOpenAIImage = request.Settings;
                await _developSettingsService.SaveAsync(settings, ct);
                _developSettingsService.MergeIntoConfig(_config.CurrentValue, settings);
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Failed to persist image-gen settings before validation");
            }
        }

        if (request.Settings is not null
            && string.Equals(request.Settings.AuthMethod, "ApiKey", StringComparison.OrdinalIgnoreCase)
            && !string.IsNullOrWhiteSpace(request.ApiKey))
        {
            await PersistSecretsOnlyAsync(imageApiKey: request.ApiKey.Trim());
            if (_rootConfiguration is IConfigurationRoot root)
            {
                try { root.Reload(); } catch (Exception ex) { _logger.LogWarning(ex, "Config reload failed"); }
            }
        }

        string? smokePath = null;
        if (request.RunSmokeTest)
        {
            var logsDir = Path.GetFullPath(Path.Combine(_env.ContentRootPath, "..", "..", "Logs"));
            try { Directory.CreateDirectory(logsDir); } catch { }
            smokePath = Path.Combine(logsDir, $"image-gen-validate-{DateTime.Now:yyyyMMdd-HHmmss}.png");
        }

        return await _imageGenService.ValidateAsync(request.RunSmokeTest, smokePath, ct);
    }
    ...
    /// <summary>
    /// Merges updated VirtualDevTeam config values into appsettings.json, preserving
    /// non-VirtualDevTeam sections (e.g., Logging). Strips secrets (PAT, API keys)
    ...
    public async Task SaveConfigAsync(VirtualDevTeamConfig updatedConfig)
    {
        ...
        try
        {
            ...
            // Hot-reload gate configuration so changes take effect immediately
            // without waiting for file-watcher reload (which doesn't update IOptions<> singletons).
            _gateCheckService?.UpdateGates(updatedConfig.HumanInteraction);
        ...
        string? imageApiKey = null)
    {
        ...
        var runnerDir = Path.GetFullPath(Path.Combine(dashboardDir, "..", "VirtualDevTeam.Runner"));
        ...
        string? caveats, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            // The glob patterns below (virtualdevteam_*.db*, sme-definitions*) are intentionally
            // narrow so develop-settings.json is never matched.
            try
            {
                var runnerDir = Path.GetDirectoryName(_appSettingsPath) ?? ".";
                var dbFiles = Directory.GetFiles(runnerDir, "virtualdevteam_*.db*");
            ...
            {
                // First, unlock and remove git worktrees that reference workspace paths.
                // Worktrees create locks that prevent normal directory deletion.
                // This mirrors the logic in scripts/minimal-reset.ps1.
                try
                {
                    var repoRoot = FindGitRepoRoot(workspaceRoot);
                    if (repoRoot is not null)
                    {
                        // Remove .lock and locked files from .git/worktrees/
                        var worktreesDir = Path.Combine(repoRoot, ".git", "worktrees");
                        if (Directory.Exists(worktreesDir))
                        {
                            foreach (var lockFile in Directory.GetFiles(worktreesDir, "locked", SearchOption.AllDirectories))
                            {
                                File.Delete(lockFile);
                                _logger.LogDebug("Removed worktree lock: {File}", lockFile);
                            }
                            foreach (var lockFile in Directory.GetFiles(worktreesDir, "*.lock", SearchOption.AllDirectories))
                            {
                                File.Delete(lockFile);
                                _logger.LogDebug("Removed worktree lock: {File}", lockFile);
                            }
                        ...
                        // Force-remove worktrees that point into the workspace
                        var enumOptions = new EnumerationOptions
                        {
                            RecurseSubdirectories = true,
                            IgnoreInaccessible = true
                        };
                        foreach (var gitFile in Directory.EnumerateFiles(workspaceRoot, ".git", enumOptions))
                        {
                            try
                            {
                                ...
                                {
                                    var worktreePath = Path.GetDirectoryName(gitFile)!;
                                    RunGitCommand(repoRoot, $"worktree remove --force \"{worktreePath}\"");
                                }
                            }
                            catch (Exception fileEx)
                            {
                                _logger.LogDebug(fileEx, "Skipping inaccessible .git file: {File}", gitFile);
                            }
                        }

                        // Prune stale worktree references
                        RunGitCommand(repoRoot, "worktree prune");
                    }
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Worktree unlock failed (non-fatal, will attempt directory delete anyway)");
                }
    ...
    /// <summary>
    /// Finds the git repository root containing the given path by walking up the directory tree.
    /// Handles both normal repos (.git is a directory) and worktree checkouts (.git is a file).
    /// </summary>
    ...
    /// <summary>
    /// Runs a git command in the specified working directory, swallowing errors.
    /// Kills the process if it exceeds the 10-second timeout.
    /// </summary>
    private void RunGitCommand(string workingDir, string arguments)
    {
        try
        {
            ...
            if (process is not null && !process.WaitForExit(10_000))
            {
                try { process.Kill(entireProcessTree: true); }
                catch { /* best effort */ }
                _logger.LogDebug("Git command timed out (killed): git {Args}", arguments);
            }
        }
        catch (Exception ex)
        {
            _logger.LogDebug(ex, "Git command failed: git {Args}", arguments);
        }
    }
}
