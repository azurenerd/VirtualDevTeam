...
public sealed record AgentSnapshot
{
    ...
    /// <summary>
    /// PR number this agent is currently working on (own implementation PR for engineers,
    /// doc PR for Researcher/Architect/PM). Null when idle/pre-PR.
    /// Powers the "🔗 PR #N" badge on agent cards and detail page.
    /// </summary>
    public int? CurrentPrNumber { get; init; }

    /// <summary>
    /// External web URL (GitHub or ADO) for <see cref="CurrentPrNumber"/>, pre-resolved
    /// at snapshot time so the dashboard doesn't need to know the platform.
    /// </summary>
    public string? CurrentPrUrl { get; init; }

    /// <summary>
    /// Structured details about why this agent is Blocked (gate id, gate name, blocked-since,
    /// PR/issue). Null unless <see cref="Status"/> is Blocked. Powers the "🚦 Review needed →"
...
public sealed class DashboardDataService : BackgroundService, IDashboardDataService
{
    ...
    private readonly VirtualDevTeam.Core.AI.RoleContextProvider? _roleContextProvider;
    private readonly VirtualDevTeam.Core.AI.ActiveLlmCallTracker? _llmCallTracker;
    private readonly VirtualDevTeam.Core.Agents.Decisions.IDecisionLog? _decisionLog;
    ...
    public DashboardDataService(
        ...
        VirtualDevTeam.Core.AI.RoleContextProvider? roleContextProvider = null,
        VirtualDevTeam.Core.AI.ActiveLlmCallTracker? llmCallTracker = null,
        VirtualDevTeam.Core.Agents.Decisions.IDecisionLog? decisionLog = null)
    {
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _timeline.RecordMilestone("🚀", "Session Started", "VirtualDevTeam pipeline initialized", "phase");

        SeedCache();

        _logger.LogInformation("Dashboard data service started");

        // Flag: seed timeline with historical phase transitions from checkpoint restore
        // on the first periodic tick (not here — the checkpoint hasn't been restored yet
        // at this point; RunCoordinator.RecoverAsync runs after DashboardDataService starts).
        bool timelineSeededFromCheckpoint = false;
        bool activityLogRestored = false;
        ...
        try
        {
            ...
            {
                // Seed timeline with historical phase transitions from checkpoint restore.
                // Deferred to the first tick because RunCoordinator.RecoverAsync runs AFTER
                // DashboardDataService.ExecuteAsync starts — GetTransitionHistory() is empty
                // at startup but populated by the time the first 10s tick fires.
                if (!timelineSeededFromCheckpoint)
                {
                    var history = _workflow.GetTransitionHistory();
                    if (history.Count > 0)
                    {
                        foreach (var transition in history)
                            _timeline.HandlePhaseChanged(transition);
                        timelineSeededFromCheckpoint = true;
                    }
                }

                // Restore persisted decisions from the prior run on the first tick.
                // Deferred for the same reason as timeline seeding: RunCoordinator.RecoverAsync
                // completes between startup and the first tick, ensuring the DB is ready.
                if (_decisionLog is not null && !_decisionLog.GetAllDecisions().Any())
                    await _decisionLog.RestoreFromDatabaseAsync(stoppingToken);

                // Restore PR/doc/review/test milestones from the activity_log on the first
                // tick. Runs unconditionally so fresh runs (no entries → no-op) and restarts
                // (entries present → milestones replayed at original timestamps) both work.
                if (!activityLogRestored)
                {
                    await _timeline.RestoreFromActivityLogAsync(
                        _stateStore,
                        _workflow.RunId,
                        _snapshots.GetAgentDisplayName,
                        stoppingToken);
                    activityLogRestored = true;
                }
    ...
    public int GetTotalPremiumRequests() => _snapshots.GetTotalPremiumRequests();
    public IReadOnlyDictionary<string, AgentUsageStats> GetAgentUsageStats() => _snapshots.GetAgentUsageStats();

    // ── IDashboardDataService: Cache management ──

    public Task InvalidatePlatformCachesAsync(CancellationToken ct = default)
    {
        InvalidatePlatformCaches();
        return Task.CompletedTask;
    }

    public void InvalidatePlatformCaches()
    {
        ...
        _logger.LogDebug("Dashboard platform caches invalidated");
        NotifyStateChanged();
    }
    ...
    public AgentRoleDescriptionInfo? GetAgentRoleDescription(string agentId)
    {
        ...
        // Fall back to built-in role description when config is empty (the default)
        if (string.IsNullOrWhiteSpace(configuredDescription))
            configuredDescription = GetDefaultRoleDescription(identity.Role);
    ...
    private static string GetDefaultRoleDescription(AgentRole role) => role switch
    {
    ...
    private void OnLlmCallChanged(object? sender, VirtualDevTeam.Core.AI.ActiveLlmCallTracker.LlmCallChangedEventArgs e)
    {
    ...
    public async Task<IReadOnlyList<PlatformPullRequest>> GetPullRequestsAsync()
    {
        ...
        try
        {
            ...
            var allPrs = await _pullRequestService.ListAllForProjectAsync(cts.Token);
            if (allPrs != null)
            {
    ...
    public async Task<IReadOnlyList<PlatformWorkItem>> GetWorkItemsAsync()
    {
        ...
        {
            using var cts = new CancellationTokenSource(DashboardApiTimeout);
            var allItems = await _workItemService.ListAllForProjectAsync(cts.Token);
            if (allItems != null)
            {
    ...
    private void ScheduleBackgroundRefresh()
    {
        ...
        _ = Task.Run(async () =>
        {
            try
            {
                ...
                try
                {
                    var allItems = await _workItemService.ListAllForProjectAsync();
                    if (allItems != null)
                    {
                        _cachedIssues = allItems.ToList();
                        _lastIssueFetchUtc = DateTime.UtcNow;
                ...
                try
                {
                    var allPrs = await _pullRequestService.ListAllForProjectAsync();
                    if (allPrs != null)
                    {
                        _cachedPullRequests = allPrs.ToList();
                        _lastPrFetchUtc = DateTime.UtcNow;
