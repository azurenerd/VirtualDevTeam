...
public class StrategyCandidateRecord
{
    ...
    public string? VisualsFeedback { get; set; }
    public string? InitialAcFeedback { get; set; }
    public string? InitialDesignFeedback { get; set; }
    public string? InitialReadabilityFeedback { get; set; }
    public string? InitialVisualsFeedback { get; set; }
    ...
    /// <summary>Path to interaction video. Null if video capture skipped.</summary>
    public string? VideoPath { get; set; }
    /// <summary>Path to animated GIF generated from video. Null if unavailable.</summary>
    public string? AnimatedGifPath { get; set; }
    /// <summary>JSON-serialized list of screenshot paths. Null if not captured.</summary>
    public string? ScreenshotPathsJson { get; set; }
    /// <summary>JSON-serialized ScreenshotCaptureSummary. Null if dual capture didn't run.</summary>
    public string? CaptureMetricsJson { get; set; }
    /// <summary>JSON-serialized PageAnalysis. Null if CDP analysis didn't run.</summary>
    public string? PageAnalysisJson { get; set; }
    /// <summary>Base URL the app was started on (e.g., "http://localhost:5142").</summary>
...
public class AgentStateStore : IDisposable
{
    ...
    public AgentStateStore(string dbPath = "virtualdevteam.db")
    {
        DatabasePath = dbPath;
    ...
    public void Reconfigure(string newDbPath)
    {
        ...
        lock (_dbLock)
        {
            try { _connection?.Close(); } catch { /* old DB may have been deleted */ }
    ...
    private void InitializeDatabase()
    {
        ...
        cmd.CommandText = """
            ...
            CREATE TABLE IF NOT EXISTS gate_rejections (
                ...
                run_id          TEXT NOT NULL DEFAULT '_global',
    ...
    private void MigrateAddRunIdColumns()
    {
        ...
        [
            ...
            // gate_rejections table (for existing DBs created before this feature)
            """
            CREATE TABLE IF NOT EXISTS gate_rejections (
                gate_key    TEXT PRIMARY KEY,
                feedback    TEXT,
            ...
            """,
            // premium request tracking columns
            "ALTER TABLE ai_usage ADD COLUMN premium_requests INTEGER NOT NULL DEFAULT 0",
            "ALTER TABLE ai_usage ADD COLUMN api_duration_ms INTEGER NOT NULL DEFAULT 0",
            // rework notification tracking
            "ALTER TABLE gate_notifications ADD COLUMN is_reworked INTEGER NOT NULL DEFAULT 0",
            // artifact preview on approval cards
            "ALTER TABLE gate_notifications ADD COLUMN artifacts_json TEXT",
            // Media artifact columns for strategy candidates
            "ALTER TABLE strategy_candidates ADD COLUMN video_path TEXT",
            "ALTER TABLE strategy_candidates ADD COLUMN animated_gif_path TEXT",
            "ALTER TABLE strategy_candidates ADD COLUMN screenshot_paths_json TEXT",
            // Dual-capture metrics and CDP page analysis
            "ALTER TABLE strategy_candidates ADD COLUMN capture_metrics_json TEXT",
            "ALTER TABLE strategy_candidates ADD COLUMN page_analysis_json TEXT",
            // App base URL for capture diagnostics
            "ALTER TABLE strategy_candidates ADD COLUMN app_base_url TEXT",
            // PR linkage on strategy task (frameworks-pr-link feature)
            "ALTER TABLE strategy_tasks ADD COLUMN pr_number INTEGER",
            "ALTER TABLE strategy_tasks ADD COLUMN pr_url TEXT",
            "ALTER TABLE strategy_tasks ADD COLUMN pr_title TEXT",
            "ALTER TABLE strategy_tasks ADD COLUMN task_title TEXT",
            // Rework iteration counter on gate_rejections (approvals rework-state persistence)
            "ALTER TABLE gate_rejections ADD COLUMN iteration_count INTEGER NOT NULL DEFAULT 1",
        ];
    ...
    /// <summary>
    /// Get all activity log entries with a timestamp at or after <paramref name="since"/>,
    /// ordered oldest-first. Pass <see cref="DateTime.MinValue"/> to return all entries.
    /// Used by the timeline restore path on runner restart.
    /// </summary>
    public async Task<IReadOnlyList<ActivityLogEntry>> GetActivityLogSinceAsync(
        DateTime since,
        CancellationToken ct = default)
    {
        await using var cmd = _connection.CreateCommand();
    ...
    /// <summary>
    /// Get all activity log entries where event_type = 'decision', ordered oldest-first.
    /// Used by <see cref="VirtualDevTeam.Core.Agents.Decisions.DecisionLog"/> to restore
    /// the in-memory decision log after a runner restart.
    /// </summary>
    public async Task<IReadOnlyList<ActivityLogEntry>> GetDecisionActivitiesAsync(CancellationToken ct = default)
    {
        await using var cmd = _connection.CreateCommand();
        cmd.CommandText = """
            SELECT id, agent_id, timestamp, event_type, details
    ...
    public async Task ClearRunStateAsync(string runId, CancellationToken ct = default)
    {
        ...
        cmd.CommandText = """
            ...
            DELETE FROM gate_rejections WHERE run_id = @runId;
            """;
        cmd.Parameters.AddWithValue("@runId", runId);
        await cmd.ExecuteNonQueryAsync(ct);
    }
    ...
    {
        SaveAiUsage(agentId, promptTokens, completionTokens, totalCalls, estimatedCost, lastModel, 0, 0);
    }

    public void SaveAiUsage(string agentId, int promptTokens, int completionTokens, int totalCalls, decimal estimatedCost, string? lastModel, int premiumRequests, long apiDurationMs)
    {
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = """
            INSERT INTO ai_usage (agent_id, prompt_tokens, completion_tokens, total_calls, estimated_cost, last_model, premium_requests, api_duration_ms, updated_at)
                VALUES (@id, @pt, @ct, @tc, @ec, @lm, @pr, @ad, datetime('now'))
            ON CONFLICT(agent_id) DO UPDATE SET
        ...
        cmd.Parameters.AddWithValue("@pr", premiumRequests);
        cmd.Parameters.AddWithValue("@ad", apiDurationMs);
        cmd.ExecuteNonQuery();
    }

    /// <summary>Load all persisted AI usage stats (for restoring after restart).</summary>
    public Dictionary<string, (int PromptTokens, int CompletionTokens, int TotalCalls, decimal EstimatedCost, string? LastModel, int PremiumRequests, long ApiDurationMs)> LoadAllAiUsage()
    {
        var result = new Dictionary<string, (int, int, int, decimal, string?, int, long)>();
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "SELECT agent_id, prompt_tokens, completion_tokens, total_calls, estimated_cost, last_model, COALESCE(premium_requests, 0), COALESCE(api_duration_ms, 0) FROM ai_usage";
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
        {
            result[reader.GetString(0)] = (
    ...
    /// <summary>Delete a specific gate approval by key.</summary>
    public void DeleteGateApproval(string key)
    {
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "DELETE FROM gate_approvals WHERE gate_id = @key;";
    ...
    /// <summary>Save a gate rejection to SQLite.</summary>
    public void SaveGateRejection(string key, string? feedback, DateTime rejectedAt, int iterationCount = 1, string runId = "_global")
    {
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = """
            INSERT INTO gate_rejections (gate_key, feedback, rejected_at, run_id, iteration_count)
                VALUES (@key, @feedback, @at, @runId, @iter)
        ...
        cmd.Parameters.AddWithValue("@at", rejectedAt);
        cmd.Parameters.AddWithValue("@runId", runId);
        cmd.Parameters.AddWithValue("@iter", iterationCount);
        cmd.ExecuteNonQuery();
    }

    /// <summary>Load all persisted gate rejections.</summary>
    public List<(string Key, string? Feedback, DateTime RejectedAt, int IterationCount)> LoadGateRejections()
    {
        var result = new List<(string, string?, DateTime, int)>();
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "SELECT gate_key, feedback, rejected_at, iteration_count FROM gate_rejections;";
        using var reader = cmd.ExecuteReader();
        while (reader.Read())
        {
            result.Add((
    ...
    /// <summary>Delete a specific gate rejection by key.</summary>
    public void DeleteGateRejection(string key)
    {
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "DELETE FROM gate_rejections WHERE gate_key = @key;";
        cmd.Parameters.AddWithValue("@key", key);
        cmd.ExecuteNonQuery();
    }

    /// <summary>Clear all gate rejections (for reset).</summary>
    public void ClearGateRejections()
    {
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = "DELETE FROM gate_rejections;";
        cmd.ExecuteNonQuery();
    }
    ...
    public void SaveGateNotification(string id, string gateId, string gateName, string context,
        int? resourceNumber, string? resourceType, string? githubUrl, bool isReworked = false,
        string? artifactsJson = null)
    {
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = """
            INSERT OR IGNORE INTO gate_notifications (id, gate_id, gate_name, context, resource_number, resource_type, github_url, is_reworked, artifacts_json)
            VALUES (@id, @gateId, @gateName, @context, @resNum, @resType, @url, @reworked, @artifactsJson);
            """;
        ...
        cmd.Parameters.AddWithValue("@reworked", isReworked ? 1 : 0);
        cmd.Parameters.AddWithValue("@artifactsJson", (object?)artifactsJson ?? DBNull.Value);
        cmd.ExecuteNonQuery();
    }
    ...
    public List<(string Id, string GateId, string GateName, string Context, int? ResourceNumber,
        string? ResourceType, string? GitHubUrl, bool IsRead, bool IsResolved, DateTime CreatedAt, DateTime? ResolvedAt, bool IsReworked, string? ArtifactsJson)>
        LoadGateNotifications()
    {
        var result = new List<(string, string, string, string, int?, string?, string?, bool, bool, DateTime, DateTime?, bool, string?)>();
        using var cmd = _connection.CreateCommand();
        cmd.CommandText = """
            SELECT id, gate_id, gate_name, context, resource_number, resource_type, github_url,
                   is_read, is_resolved, created_at, resolved_at, is_reworked, artifacts_json
        ...
        while (reader.Read())
        {
            result.Add((
                ...
                reader.IsDBNull(11) ? false : reader.GetInt32(11) != 0,
        ...
        try
        {
            ...
            taskCmd.CommandText = """
                INSERT OR REPLACE INTO strategy_tasks
                    (run_id, task_id, task_title, started_at, completed_at, winner_strategy_id, tie_break_reason, evaluation_elapsed_sec, pr_number, pr_url, pr_title)
                VALUES
                    ($run_id, $task_id, $task_title, $started_at, $completed_at, $winner, $tiebreak, $eval_sec, $pr_number, $pr_url, $pr_title)
                """;
            taskCmd.Parameters.AddWithValue("$run_id", task.RunId);
            taskCmd.Parameters.AddWithValue("$task_id", task.TaskId);
            taskCmd.Parameters.AddWithValue("$task_title", task.TaskTitle ?? (object)DBNull.Value);
            ...
            taskCmd.Parameters.AddWithValue("$pr_number", task.PrNumber.HasValue ? (object)task.PrNumber.Value : DBNull.Value);
            taskCmd.Parameters.AddWithValue("$pr_url", task.PrUrl ?? (object)DBNull.Value);
            taskCmd.Parameters.AddWithValue("$pr_title", task.PrTitle ?? (object)DBNull.Value);
            ...
            foreach (var c in task.Candidates)
            {
                ...
                cCmd.CommandText = """
                    INSERT OR REPLACE INTO strategy_candidates
                        (run_id, task_id, strategy_id, state, started_at, completed_at,
                         ...
                         video_path, animated_gif_path, screenshot_paths_json,
                         capture_metrics_json, page_analysis_json, app_base_url)
                    VALUES
                        ($run_id, $task_id, $strategy_id, $state, $started_at, $completed_at,
                         $elapsed_sec, $succeeded, $failure_reason, $tokens_used,
                         $ac_score, $design_score, $readability_score, $visuals_score, $survived,
                ...
                cCmd.Parameters.AddWithValue("$video_path", c.VideoPath ?? (object)DBNull.Value);
                cCmd.Parameters.AddWithValue("$animated_gif_path", c.AnimatedGifPath ?? (object)DBNull.Value);
                cCmd.Parameters.AddWithValue("$screenshot_paths_json", c.ScreenshotPathsJson ?? (object)DBNull.Value);
                cCmd.Parameters.AddWithValue("$capture_metrics_json", c.CaptureMetricsJson ?? (object)DBNull.Value);
                cCmd.Parameters.AddWithValue("$page_analysis_json", c.PageAnalysisJson ?? (object)DBNull.Value);
                cCmd.Parameters.AddWithValue("$app_base_url", c.AppBaseUrl ?? (object)DBNull.Value);
    ...
    public List<StrategyTaskRecord> LoadRecentStrategyTasks(int limit = 100)
    {
        ...
        taskCmd.CommandText = """
            SELECT run_id, task_id, started_at, completed_at, winner_strategy_id,
                   tie_break_reason, evaluation_elapsed_sec, pr_number, pr_url, pr_title,
                   task_title
            FROM strategy_tasks
        ...
        while (taskReader.Read())
        {
            ...
            var task = new StrategyTaskRecord
            {
                ...
                PrNumber = taskReader.IsDBNull(7) ? null : taskReader.GetInt32(7),
                PrUrl = taskReader.IsDBNull(8) ? null : taskReader.GetString(8),
                PrTitle = taskReader.IsDBNull(9) ? null : taskReader.GetString(9),
                TaskTitle = taskReader.IsDBNull(10) ? null : taskReader.GetString(10),
        ...
        foreach (var task in tasks)
        {
            ...
            while (cReader.Read())
            {
                task.Candidates.Add(new StrategyCandidateRecord
                {
                    ...
                    VideoPath = cReader.FieldCount > 24 && !cReader.IsDBNull(24) ? cReader.GetString(24) : null,
                    AnimatedGifPath = cReader.FieldCount > 25 && !cReader.IsDBNull(25) ? cReader.GetString(25) : null,
                    ScreenshotPathsJson = cReader.FieldCount > 26 && !cReader.IsDBNull(26) ? cReader.GetString(26) : null,
                    CaptureMetricsJson = cReader.FieldCount > 27 && !cReader.IsDBNull(27) ? cReader.GetString(27) : null,
                    PageAnalysisJson = cReader.FieldCount > 28 && !cReader.IsDBNull(28) ? cReader.GetString(28) : null,
                    AppBaseUrl = cReader.FieldCount > 29 && !cReader.IsDBNull(29) ? cReader.GetString(29) : null,
                });
            }
    ...
    // ── Generic SQL helpers for satellite stores (TaskStateMachine, etc.) ──

    /// <summary>
    /// Execute a non-query SQL statement with optional parameters. Returns rows affected.
    /// Thread-safe via <c>_dbLock</c>.
    /// </summary>
    ...
    /// <summary>
    /// Execute a scalar SQL query with optional parameters. Returns the first column of the first row.
    /// Thread-safe via <c>_dbLock</c>.
    /// </summary>
    ...
    /// <summary>
    /// Execute a SQL query and process each row via a callback. Thread-safe via <c>_dbLock</c>.
    /// </summary>
    public void ExecuteReader(string sql, Action<Microsoft.Data.Sqlite.SqliteDataReader> rowAction, params (string Name, object Value)[] parameters)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        lock (_dbLock)
        {
