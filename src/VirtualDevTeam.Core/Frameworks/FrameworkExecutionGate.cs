...
public sealed class FrameworkExecutionGate
{
    ...
    /// visibility when delegating to a framework that VirtualDevTeam cannot inspect mid-flight.
    /// </summary>
    ...
    {
        // post-run-squad-crash-retry: differentiate runtime crashes from regular failures.
        // Files-modified count from a crashed framework is meaningless (partial corruption);
        // pretending the patch will be "evaluated by the standard pipeline" misleads operators.
        var isRuntimeCrash = !result.Succeeded
            && (result.FailureReason ?? "").StartsWith("runtime-crash:", StringComparison.Ordinal);

        var status = result.Succeeded
            ? "completed successfully"
            : isRuntimeCrash
                ? $"CRASHED ({result.FailureReason})"
                : $"failed: {result.FailureReason}";
        var tokenInfo = result.TokensUsed.HasValue
            ? $", tokens: {result.TokensUsed.Value:N0}"
            : ", tokens: unknown";

        var summary = isRuntimeCrash
            ? $"{frameworkId} {status} in {result.Elapsed.TotalSeconds:F1}s{tokenInfo}. " +
              "Partial output discarded; orchestrator will rely on parallel strategies (or legacy fallback)."
            : $"{frameworkId} {status} in {result.Elapsed.TotalSeconds:F1}s{tokenInfo}. " +
