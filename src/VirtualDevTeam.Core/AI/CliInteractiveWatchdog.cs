...
public sealed partial class CliInteractiveWatchdog
{
    ...
    private static bool IsCredentialPrompt(string line)
    {
        ...
        var match = CredentialPatternRegex().Match(line);
        if (!match.Success) return false;

        // --- Exclude code/YAML/config content that mentions credential keywords ---

        // Indented lines are code/YAML structure, not interactive prompts
        if (line.Length > 0 && char.IsWhiteSpace(line[0])) return false;

        // Template/expression syntax (GitHub Actions, Handlebars, etc.)
        if (line.Contains("${{") || line.Contains("{{") || line.Contains("}}")) return false;

        // Keyword is part of a compound identifier (e.g., id-token, _token, GITHUB_TOKEN)
        // Real prompts use standalone words: "Password:", "Enter your token:", "API key:"
        if (match.Index > 0)
        {
            var before = line[match.Index - 1];
