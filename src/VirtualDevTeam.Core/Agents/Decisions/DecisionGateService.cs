...
public class DecisionGateService
{
        ...
        CancellationToken ct = default)
    {
        ...
        // Store the decision and create the gate notification BEFORE generating the plan.
        // This ensures the Approvals page can see and act on the decision immediately,
        // even while the (potentially slow) plan-generation LLM call is still running.
        _decisionLog.Log(decision);
        ...
        if (requiresGate)
        {
            ...
            // Turn 2 (optional): Generate structured plan for human review context.
            // Runs after the decision is already visible on the Approvals page.
            if (_config.RequirePlanForGated && _config.MaxDecisionTurns >= 2)
            {
                try
                {
                    var plan = await GeneratePlanAsync(agentId, agentDisplayName, title, context, rationale,
                        alternatives, affectedFiles, riskAssessment, modelTier, ct);
                    decision.Plan = plan;
                    _decisionLog.Log(decision); // re-log to update with plan
                }
                catch (Exception ex)
                {
                    _logger.LogWarning(ex, "Plan generation failed for decision {Id} — decision remains gated without plan", decision.Id);
                }
            }
        }
        else
    ...
    /// </summary>
    public void ApproveDecision(string decisionId, string? feedback = null, string? approvedBy = null)
    {
        _decisionLog.Update(decisionId, DecisionStatus.Approved, feedback, approvedBy ?? "human");
        _gateNotificationService.Resolve($"{DecisionGatePrefix}{decisionId}");
        _logger.LogInformation("Decision {Id} approved by {Actor} (feedback: {Feedback})",
            decisionId, approvedBy ?? "human", feedback ?? "none");
    }
