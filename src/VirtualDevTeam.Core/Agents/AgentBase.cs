...
public abstract class AgentBase : IAgent, IDisposable
{
    ...
    private BlockedReason? _currentBlockedReason;
    ...
    /// <summary>
    /// Wake signal for the agent loop. Bus message handlers release this semaphore
    /// to interrupt the poll-interval delay and process work immediately.
    /// </summary>
    ...
    /// <summary>
    /// PR number currently being worked on by this agent. Engineers set this to their
    /// implementation PR; Researcher/Architect/PM set it to their document PR.
    /// Surfaced on the dashboard so operators can jump straight to the agent's active PR.
    /// </summary>
    public int? CurrentPrNumber { get; protected set; }

    /// <summary>
    /// Structured reason for the agent being Blocked on a human gate, or null.
    /// Set automatically by <see cref="WaitForHumanGateAsync"/>; cleared when the gate resolves.
    /// </summary>
    public BlockedReason? CurrentBlockedReason
    {
        get { lock (_statusLock) { return _currentBlockedReason; } }
    }
    ...
    protected void UpdateStatus(AgentStatus newStatus, string? reason = null)
    {
        ...
        // NoMessyCodePlan Theme 6: a true status TRANSITION (oldStatus != newStatus) gets
        // an Info log — that's the lifecycle signal operators want. A reason-only update
        // (status unchanged, reason changed — e.g. "Working on step 2" → "Working on step 3")
        // is intra-loop chatter; dashboards already render the current reason from the
        // agent's state. Demote those to Debug so the operator log stays signal-only.
        ...
        {
            Logger.LogDebug("Agent {AgentId} status reason updated: {OldReason} -> {NewReason}",
                Identity.Id, oldReason ?? "(none)", reason ?? "(none)");
        }
    ...
    /// <summary>
    /// Waits for a human gate, setting agent status to Blocked while waiting.
    /// Automatically restores Working status when the gate is resolved.
    /// If the gate auto-approves (not requiring human), status is unchanged.
    /// </summary>
    protected async Task<GateWaitResult> WaitForHumanGateAsync(
        string gateId,
        string context,
        ...
        CancellationToken ct = default)
    {
        var gateCheck = Core?.GateCheck ?? throw new InvalidOperationException("GateCheck service not available");

        // Quick check: if gate doesn't require human, skip immediately
        if (!gateCheck.RequiresHuman(gateId))
        {
            return await gateCheck.WaitForGateAsync(gateId, context, resourceNumber, pollIntervalSeconds, ct);
        }

        // Gate requires human — set Blocked status + structured BlockedReason while waiting.
        var gateName = ResolveGateName(gateId);
        lock (_statusLock)
        {
            _currentBlockedReason = new BlockedReason
            {
                GateId = gateId,
                GateName = gateName,
                BlockedSince = DateTime.UtcNow,
                PullRequestNumber = resourceNumber ?? CurrentPrNumber,
                IssueNumber = null
            };
        }
        UpdateStatus(AgentStatus.Blocked, $"Awaiting human approval: {gateId}");

        // Mark the current task step as waiting (if one is active) so the
        // dashboard shows "Waiting" instead of "In Progress" during gate polls.
        var tracker = Core?.TaskTracker;
        var currentStep = tracker?.GetCurrentStep(Identity.Id);
        if (currentStep is not null)
            tracker!.SetStepWaiting(currentStep.Id);
        ...
        {
            // Restore step to InProgress so subsequent work shows correctly on the dashboard.
            // CompleteStep will override this anyway, but the intermediate state matters for
            // agents that do significant work between gate resolution and step completion.
            if (currentStep is not null && currentStep.Status == Agents.Steps.AgentTaskStepStatus.WaitingOnHuman)
                currentStep.Status = Agents.Steps.AgentTaskStepStatus.InProgress;

            lock (_statusLock) { _currentBlockedReason = null; }
            UpdateStatus(AgentStatus.Working, "Human gate resolved, resuming work");
        }
    }

    /// <summary>
    /// Resolves a gate's display name from <see cref="Configuration.GateIds.AllGates"/>.
    /// Falls back to the raw gate id if not found in the table.
    /// </summary>
    private static string ResolveGateName(string gateId)
    {
        var match = Configuration.GateIds.AllGates.FirstOrDefault(g =>
            string.Equals(g.Id, gateId, StringComparison.OrdinalIgnoreCase));
        return match.Name ?? gateId;
    }
    ...
    protected void Subscribe<T>(Func<T, CancellationToken, Task> handler) where T : AgentMessage
    {
        ...
        // Wrap the handler to auto-wake the agent loop after processing
        async Task WakeAfterHandle(T msg, CancellationToken ct)
        {
            await handler(msg, ct);
            WakeLoop();
        }
        _messageSubscriptions.Add(Core!.MessageBus.Subscribe<T>(Identity.Id, WakeAfterHandle));
    }
    ...
    /// <summary>
    /// Signal the agent loop to wake up immediately instead of waiting for the poll interval.
    /// Called automatically by <see cref="Subscribe{T}"/> after each message handler completes.
    /// Can also be called manually (e.g., after a gate approval or external event).
    /// </summary>
    protected void WakeLoop() => _wakeSignal.Release();

    /// <summary>
    /// Wait for either a wake signal (from a bus message handler) or the specified timeout.
    /// Use this instead of <c>Task.Delay</c> in agent loops so that bus messages interrupt
    /// the poll interval and trigger immediate processing.
    /// </summary>
    protected async Task WaitForWakeOrTimeoutAsync(TimeSpan timeout, CancellationToken ct)
    {
        // Returns immediately if a signal is pending, otherwise waits up to timeout.
        // OperationCanceledException propagates if ct is cancelled.
