...
public class WorkspaceConfig
{
    ...
    /// <summary>
    /// When true (default), run MCP and Direct C# Playwright capture in parallel during
    /// strategy evaluation. MCP provides AI-driven adaptive exploration; Direct provides
    /// fast deterministic capture. Both are read-only and safe to run simultaneously.
    /// When false, only Direct C# Playwright capture runs (legacy behavior).
    /// </summary>
    public bool DualCaptureEnabled { get; set; } = true;
    ...
    /// </summary>
    public string GitUserName { get; set; } = "VirtualDevTeam";

    /// <summary>
    /// Git email for commits in local workspaces.
    /// </summary>
    public string GitUserEmail { get; set; } = "virtualdevteam@noreply.github.com";

    #region Workspace Mode (Large Project Support)

    /// <summary>
    /// How agent workspaces are created: Clone (full clone per agent, default),
    /// Worktree (single shared clone + lightweight worktrees),
    /// InPlace (operator's existing checkout + worktrees).
    /// </summary>
    public WorkspaceMode WorkspaceMode { get; set; } = WorkspaceMode.InPlace;
    ...
    /// Must be on the same filesystem as the source repo for hardlink sharing.
    /// </summary>
    ...
    /// <summary>Whether the workspace mode uses worktrees (Worktree or InPlace).</summary>
    public bool IsWorktreeMode => WorkspaceMode is WorkspaceMode.Worktree or WorkspaceMode.InPlace;

    /// <summary>Whether the workspace mode is InPlace (operator's existing checkout).</summary>
    public bool IsInPlaceMode => WorkspaceMode == WorkspaceMode.InPlace;

    /// <summary>
    /// When set, agent branch pushes go to this path/URL instead of 'origin'.
    /// Used in Local DevPlatform mode to redirect pushes to the local bare repo
    /// so agent branches don't spam the real GitHub/ADO remote.
    /// Only the final submission service pushes to the real remote.
    /// </summary>
    public string? AgentPushRemote { get; set; }

    /// <summary>Whether agent pushes should go to a local target instead of origin.</summary>
    public bool HasLocalPushTarget => !string.IsNullOrWhiteSpace(AgentPushRemote);
