...
/// <summary>
/// **LEGACY (NoMessyCodePlan Theme 3) — DO NOT USE FROM NEW AGENT CODE.**
/// Use the capability interfaces in <see cref="VirtualDevTeam.Core.DevPlatform.Capabilities"/>
/// instead:
/// <list type="bullet">
///   <item><see cref="DevPlatform.Capabilities.IPullRequestService"/> for PR operations</item>
///   <item><see cref="DevPlatform.Capabilities.IWorkItemService"/> for issues / work items</item>
///   <item><see cref="DevPlatform.Capabilities.IReviewService"/> for reviews / comments</item>
///   <item><see cref="DevPlatform.Capabilities.IRepositoryContentService"/> for repo files</item>
///   <item><see cref="DevPlatform.Capabilities.IBranchService"/> for branches</item>
/// </list>
/// <para>
/// <c>IGitHubService</c> is the internal adapter layer that the GitHub capability providers
/// (e.g. <c>GitHubPullRequestAdapter</c>) wrap. Direct use from agents bypasses the
/// platform-neutral abstraction that lets the same code run against Azure DevOps.
/// </para>
/// <para>
/// See <c>docs/CAPABILITY_BOUNDARIES.md</c> for the full migration policy.
/// </para>
/// </summary>
[Obsolete("Use capability interfaces (IPullRequestService, IWorkItemService, IReviewService, " +
    "IRepositoryContentService, IBranchService) instead — IGitHubService is the internal " +
    "adapter layer. See docs/CAPABILITY_BOUNDARIES.md.", error: false)]
public interface IGitHubService
{
    ...
    /// <summary>List all PRs without any run-scope filter. Use for display/history only.</summary>
    Task<IReadOnlyList<AgentPullRequest>> GetAllPullRequestsUnfilteredAsync(CancellationToken ct = default);
    ...
    /// <summary>List all issues without any run-scope filter. Use for display/history only.</summary>
    Task<IReadOnlyList<AgentIssue>> GetAllIssuesUnfilteredAsync(CancellationToken ct = default);
    ...
    /// <summary>
    /// Uploads an image as a release asset to a hidden "screenshots-store" release.
    /// Returns a permanent URL suitable for embedding in PR/issue markdown comments.
    /// Unlike CommitBinaryFileAsync, the returned URL does not expire for private/EMU repos.
    /// </summary>
    Task<string?> UploadImageAsReleaseAssetAsync(
