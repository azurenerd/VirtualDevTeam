...
public class GitHubService : IGitHubService
{
    ...
    // Multiple agents poll GetOpenIssuesAsync/GetOpenPullRequestsAsync every few seconds.
    // Without caching, 7 agents x 2-5 list calls/cycle burns ~5000 API calls/17 min.
    // TTLs are configurable via VirtualDevTeam:GitHubCache in appsettings.json.
    private readonly IOptionsMonitor<VirtualDevTeamConfig> _config;
    private GitHubCacheConfig CacheConfig => _config.CurrentValue.GitHubCache;

    // Effective TTL per cache tier -- returns TimeSpan.Zero (always-miss) when Enabled=false.
    ...
    // Unfiltered (no run-scope) — used by dashboard/timeline display
    private IReadOnlyList<AgentIssue>? _allIssuesUnfilteredCache;
    private DateTime _allIssuesUnfilteredCacheTime;
    private readonly SemaphoreSlim _allIssuesUnfilteredLock = new(1, 1);
    ...
    // Unfiltered (no run-scope) — used by dashboard/timeline display
    private IReadOnlyList<AgentPullRequest>? _allPrsUnfilteredCache;
    private DateTime _allPrsUnfilteredCacheTime;
    private readonly SemaphoreSlim _allPrsUnfilteredLock = new(1, 1);
    ...
    public GitHubService(IOptionsMonitor<VirtualDevTeamConfig> config, RateLimitManager rateLimitManager, ILogger<GitHubService> logger,
        AgentStateStore? stateStore = null)
    {
        ...
        var projectConfig = config.CurrentValue.Project;
        ...
        if (string.IsNullOrWhiteSpace(repoValue) || !repoValue.Contains('/'))
        {
            ...
            _repo = "";
            _token = "";
            _client = new GitHubClient(new ProductHeaderValue("VirtualDevTeam"));
    ...
    private void InvalidateListCaches()
    {
        ...
        _allIssuesUnfilteredCache = null;
        _openPrsCache = null;
        _allPrsCache = null;
        _allPrsUnfilteredCache = null;
    ...
    public async Task<IReadOnlyList<AgentPullRequest>> GetOpenPullRequestsAsync(CancellationToken ct = default)
    {
        ...
        if (_openPrsCache is { } cached && DateTime.UtcNow - _openPrsCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-open-prs (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _openPrsLock.WaitAsync(ct);
        try
        {
            // Double-check after acquiring lock
            if (_openPrsCache is { } cached2 && DateTime.UtcNow - _openPrsCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-open-prs[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
    ...
    {
        if (!IsConfigured) return Array.Empty<AgentPullRequest>();
        if (_allPrsCache is { } cached && DateTime.UtcNow - _allPrsCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-all-prs (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _allPrsLock.WaitAsync(ct);
        try
        {
            if (_allPrsCache is { } cached2 && DateTime.UtcNow - _allPrsCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-all-prs[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
            ...
            var result = await _rl.ExecuteAsync(async _ =>
            {
                ...
                if (_runStartedUtc.HasValue)
                {
                    ...
                    // Also keep any PR with the AI-Generated label (covers SoftwareEngineer,
                    // spawned specialists, etc.) so the dashboard shows all agent work.
                    string[] anchorPrefixes = { "Researcher:", "ProgramManager:", "Architect:", "SoftwareEngineer:", "TestEngineer:" };
                    var anchorPrs = anchorPrefixes
                        ...
                        .ToList();
                    // Also include the most recent AI-Generated PR not already anchored
                    var aiGeneratedPrs = mapped
                        .Where(pr => pr.Labels.Contains("AI-Generated", StringComparer.OrdinalIgnoreCase))
                        .Where(pr => !anchorPrs.Any(a => a.Number == pr.Number))
                        .OrderByDescending(pr => pr.CreatedAt)
                        .Take(10)
                        .ToList();
                    var inRun = mapped.Where(pr => pr.CreatedAt >= _runStartedUtc.Value).ToList();
                    var combined = inRun
                        .Concat(anchorPrs.Where(a => !inRun.Any(p => p.Number == a.Number)))
                        .Concat(aiGeneratedPrs.Where(a => !inRun.Any(p => p.Number == a.Number) && !anchorPrs.Any(p => p.Number == a.Number)))
                        .ToList();
                    mapped = combined;
                }
                return mapped;
    ...
    public async Task<IReadOnlyList<AgentPullRequest>> GetAllPullRequestsUnfilteredAsync(CancellationToken ct = default)
    {
        if (!IsConfigured) return Array.Empty<AgentPullRequest>();
        if (_allPrsUnfilteredCache is { } cached && DateTime.UtcNow - _allPrsUnfilteredCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-all-prs-unfiltered (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _allPrsUnfilteredLock.WaitAsync(ct);
        try
        {
            if (_allPrsUnfilteredCache is { } cached2 && DateTime.UtcNow - _allPrsUnfilteredCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-all-prs-unfiltered[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
            ...
            var result = await _rl.ExecuteAsync(async _ =>
            {
                var prs = await _client.PullRequest.GetAllForRepository(_owner, _repo,
                    new PullRequestRequest { State = ItemStateFilter.All, SortDirection = SortDirection.Descending });
                TrackRateLimit();
                // No run-scope filter — return full project history for display purposes
                return (IReadOnlyList<AgentPullRequest>)prs
                    .Select(pr => MapPullRequest(pr, pr.Labels.Select(l => l.Name).ToList()))
                    .ToList();
            }, ct);

            _allPrsUnfilteredCache = result;
            _allPrsUnfilteredCacheTime = DateTime.UtcNow;
            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to get all PRs (unfiltered)");
            throw;
        }
        finally { _allPrsUnfilteredLock.Release(); }
    }
    ...
    {
        if (!IsConfigured) return Array.Empty<AgentPullRequest>();
        if (_mergedPrsCache is { } cached && DateTime.UtcNow - _mergedPrsCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-merged-prs (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _mergedPrsLock.WaitAsync(ct);
        try
        {
            if (_mergedPrsCache is { } cached2 && DateTime.UtcNow - _mergedPrsCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-merged-prs[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
        ...
        CancellationToken ct = default)
    {
        await _rl.ExecuteAsync(async _ =>
        {
            try
            {
                ...
                httpClient.DefaultRequestHeaders.Add("User-Agent", "VirtualDevTeam");
                ...
                if (!response.IsSuccessStatusCode)
                {
                    ...
                    var fullBody = body;
                    if (mappedComments.Count > 0)
                    {
                        var reason = isOwnPrError
                            ? "could not post as review on own PR"
                            : "line numbers not in diff hunks";
                        var sb = new System.Text.StringBuilder(body);
                        sb.AppendLine($"\n\n---\n**Inline comments ({reason}):**\n");
                        foreach (var c in comments)
                        {
                            sb.AppendLine($"- **`{c.FilePath}:{c.Line}`** — {c.Body}");
                        }
                        fullBody = sb.ToString();
                        _logger.LogWarning("Review fallback on PR #{Number}: {Count} inline comments moved to body ({Reason})",
                            prNumber, comments.Count, reason);
                    }
    ...
    public async Task<IReadOnlyList<Models.IssueComment>> GetPullRequestCommentsAsync(int prNumber, CancellationToken ct = default)
    {
        ...
        try
        {
            if (_prCommentsCache.TryGetValue(prNumber, out var cached) &&
                DateTime.UtcNow - cached.CachedAt < GetByNumberTtl)
                return cached.Data;
        }
    ...
    public async Task<IReadOnlyList<AgentIssue>> GetOpenIssuesAsync(CancellationToken ct = default)
    {
        ...
        if (_openIssuesCache is { } cached && DateTime.UtcNow - _openIssuesCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-open-issues (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _openIssuesLock.WaitAsync(ct);
        try
        {
            if (_openIssuesCache is { } cached2 && DateTime.UtcNow - _openIssuesCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-open-issues[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
    ...
    {
        if (!IsConfigured) return Array.Empty<AgentIssue>();
        if (_allIssuesCache is { } cached && DateTime.UtcNow - _allIssuesCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-all-issues (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _allIssuesLock.WaitAsync(ct);
        try
        {
            if (_allIssuesCache is { } cached2 && DateTime.UtcNow - _allIssuesCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-all-issues[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
    ...
    public async Task<IReadOnlyList<AgentIssue>> GetAllIssuesUnfilteredAsync(CancellationToken ct = default)
    {
        if (!IsConfigured) return Array.Empty<AgentIssue>();
        if (_allIssuesUnfilteredCache is { } cached && DateTime.UtcNow - _allIssuesUnfilteredCacheTime < ListOpenTtl)
        { _logger.LogDebug("GitHub cache HIT list-all-issues-unfiltered (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached; }

        await _allIssuesUnfilteredLock.WaitAsync(ct);
        try
        {
            if (_allIssuesUnfilteredCache is { } cached2 && DateTime.UtcNow - _allIssuesUnfilteredCacheTime < ListOpenTtl)
            { _logger.LogDebug("GitHub cache HIT list-all-issues-unfiltered[lock] (hits: {Count})", Interlocked.Increment(ref _cacheHitCount)); return cached2; }
            ...
            _allIssuesUnfilteredCache = result;
            _allIssuesUnfilteredCacheTime = DateTime.UtcNow;
            return result;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to get all issues (unfiltered)");
            throw;
        }
        finally { _allIssuesUnfilteredLock.Release(); }
    }
    ...
    public async Task<IReadOnlyList<AgentIssue>> GetIssuesByLabelAsync(string label, CancellationToken ct = default)
    {
        ...
        try
        {
            if (_labelIssuesCache.TryGetValue(cacheKey, out var entry) && DateTime.UtcNow - entry.CachedAt < ListByLabelTtl)
            {
                _logger.LogDebug("GitHub cache HIT label-issues:{Key}", cacheKey);
                return entry.Data;
            }
    ...
    public async Task UpdateIssueAsync(int issueNumber, string? title = null, string? body = null, string[]? labels = null, string? state = null, CancellationToken ct = default)
    {
        await _rl.ExecuteAsync(async _ =>
        {
            try
            {
                ...
                {
                    // 2026-05-12 fix (issue-reopen-loop on T-1482): callers were passing
                    // bogus state values like "inprogress" expecting them to set a STATUS
                    // LABEL but the GitHub adapter has only "open" or "closed" as valid
                    // issue states. The previous code path treated anything-non-"closed"
                    // as Open, which silently REOPENED closed issues on every claim cycle
                    // (Game Engine Engineer 1 → reopen → SoftwareEngineer 1 → reopen → ...).
                    // Now we require an exact match — anything else is a no-op + warning,
                    // so the state field is unchanged when callers misuse it.
                    if (string.Equals(state, "closed", StringComparison.OrdinalIgnoreCase))
                        ...
                        _logger.LogWarning(
                            "UpdateIssueAsync called with unknown state value '{State}' for issue #{Number} — ignoring. " +
                            "Valid values are \"open\" or \"closed\"; status labels (status:in-progress etc.) belong in the labels parameter.",
                            state, issueNumber);
                }
                else
                {
                    // 2026-05-13 fix (issue-reopen-via-label-update): when state is NOT
                    // explicitly passed, we MUST still set it explicitly to preserve current
                    // state. Without this, calling Update with only title/labels reopens
                    // CLOSED issues (observed: #1478, #1483, #1487 closed manually then
                    // reopened by AssignTaskToAgent/MarkInProgress/MarkImplementationComplete
                    // calling UpdateAsync without state -> implicit reopen via Octokit
                    // serialization + GitHub PATCH behavior).
                    //
                    // Pre-fetch the current state and pass it through explicitly. Adds 1
                    // GET per update but eliminates the entire reopen-via-label-update
                    // attack surface — all 5 known call sites that pass title/labels
                    // without state are protected without needing per-caller guards.
                    try
                    {
                        var current = await _client.Issue.Get(_owner, _repo, issueNumber);
                        if (current is not null)
                            update.State = current.State.Value;
                    }
                    catch (Exception ex)
                    {
                        _logger.LogDebug(ex,
                            "UpdateIssueAsync: pre-fetch of current state failed for #{Number} — " +
                            "proceeding without explicit state (closed issues may reopen)",
                            issueNumber);
                    }
                }
                if (labels is not null)
                ...
                TrackRateLimit();
                _logger.LogInformation("Updated issue #{Number} (state preserved: {State})",
                    issueNumber, update.State.HasValue ? update.State.Value.ToString() : "default");
                InvalidateListCaches();
            }
            catch (Exception ex)
            {
        ...
        CancellationToken ct = default)
    {
        return await _rl.ExecuteAsync(async _ =>
        {
            ...
            // Get the download URL from the Contents API using the BRANCH name (not commit SHA).
            // Branch-based lookup survives force-pushes — the commit SHA becomes unreachable after
            // rebases, causing 404s in PR comments. The download_url includes a temporary token
            // that allows unauthenticated access (GitHub's camo proxy fetches and caches it on
            // first render, so the image persists even after the token expires).
            try
            {
                var contents = await _client.Repository.Content.GetAllContentsByRef(
                    _owner, _repo, path, branch);
                if (contents.Count > 0 && !string.IsNullOrEmpty(contents[0].DownloadUrl))
                {
                    return contents[0].DownloadUrl;
                }
            }
            catch (Exception ex)
            {
                _logger.LogDebug(ex, "Could not get download URL with token for {Path}, falling back to raw URL", path);
            }

            // Fallback: use branch-based raw URL (survives rebases, requires auth for private repos)
            return $"https://raw.githubusercontent.com/{_owner}/{_repo}/{branch}/{path}";
        }, ct);
    }

    // --- Release-based image hosting (permanent URLs for PR/issue markdown) ---

    private const string ScreenshotReleaseTag = "screenshots-store";
    private long? _screenshotReleaseId;
    private string? _screenshotUploadUrl;
    private readonly SemaphoreSlim _releaseLock = new(1, 1);

    public async Task<string?> UploadImageAsReleaseAssetAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        return await _rl.ExecuteAsync(async _ =>
        {
            try
            {
                var uploadUrl = await GetOrCreateScreenshotReleaseUploadUrlAsync(ct);

                var assetUrl = await UploadReleaseAssetViaHttpAsync(uploadUrl, filename, content, contentType, ct);
                if (assetUrl is not null)
                {
                    _logger.LogInformation("Uploaded screenshot asset {Filename} ({Size} bytes) — permanent URL: {Url}",
                        filename, content.Length, assetUrl);
                    return assetUrl;
                }
            ...
            {
                _logger.LogWarning(ex, "Failed to upload screenshot as release asset: {Filename}", filename);
                return null;
            }
        }, ct);
    }

    private async Task<string?> UploadReleaseAssetViaHttpAsync(
        string uploadUrl, string filename, byte[] content, string contentType, CancellationToken ct)
    {
        // GitHub release upload URL template: https://uploads.github.com/repos/{owner}/{repo}/releases/{id}/assets{?name,label}
        var baseUrl = uploadUrl.Split('{')[0].TrimEnd('?');
        var requestUrl = $"{baseUrl}?name={Uri.EscapeDataString(filename)}";
        ...
        var response = await httpClient.PostAsync(requestUrl, httpContent, ct);

        if (response.IsSuccessStatusCode)
        {
            var json = await response.Content.ReadAsStringAsync(ct);
            // Parse browser_download_url from response
            var match = System.Text.RegularExpressions.Regex.Match(json, "\"browser_download_url\"\\s*:\\s*\"([^\"]+)\"");
            return match.Success ? match.Groups[1].Value : null;
        }

        var errorBody = await response.Content.ReadAsStringAsync(ct);

        // Handle duplicate filename — retry with GUID suffix
        if ((int)response.StatusCode == 422 && errorBody.Contains("already_exists", StringComparison.OrdinalIgnoreCase))
        {
            var uniqueName = Path.GetFileNameWithoutExtension(filename) + "-" + Guid.NewGuid().ToString("N")[..6] + Path.GetExtension(filename);
            _logger.LogWarning("Asset {Filename} already exists, retrying as {UniqueName}", filename, uniqueName);

            var retryUrl = $"{baseUrl}?name={Uri.EscapeDataString(uniqueName)}";
            using var retryContent = new ByteArrayContent(content);
            retryContent.Headers.ContentType = new System.Net.Http.Headers.MediaTypeHeaderValue(contentType);

            var retryResponse = await httpClient.PostAsync(retryUrl, retryContent, ct);
            if (retryResponse.IsSuccessStatusCode)
            {
                var retryJson = await retryResponse.Content.ReadAsStringAsync(ct);
                var retryMatch = System.Text.RegularExpressions.Regex.Match(retryJson, "\"browser_download_url\"\\s*:\\s*\"([^\"]+)\"");
                return retryMatch.Success ? retryMatch.Groups[1].Value : null;
            }
        }

        _logger.LogWarning("Release asset upload failed ({Status}): {Error}", (int)response.StatusCode, errorBody);
        return null;
    }

    private async Task<string> GetOrCreateScreenshotReleaseUploadUrlAsync(CancellationToken ct)
    {
        if (_screenshotUploadUrl is not null)
            return _screenshotUploadUrl;

        await _releaseLock.WaitAsync(ct);
        try
        {
            if (_screenshotUploadUrl is not null)
                return _screenshotUploadUrl;
            ...
            try
            {
                var existing = await _client.Repository.Release.Get(_owner, _repo, ScreenshotReleaseTag);
                _screenshotReleaseId = existing.Id;
                _screenshotUploadUrl = existing.UploadUrl;
                return existing.UploadUrl;
            }
            ...
            try
            {
                var newRelease = new NewRelease(ScreenshotReleaseTag)
                {
                    Name = "Screenshot Assets (internal)",
                    Body = "Internal: Screenshot attachments for PR/issue comments. Do not delete.",
                    Prerelease = true,
                    Draft = false,
                    MakeLatest = MakeLatestQualifier.False
                };
                var created = await _client.Repository.Release.Create(_owner, _repo, newRelease);
                _screenshotReleaseId = created.Id;
                _screenshotUploadUrl = created.UploadUrl;
                _logger.LogInformation("Created screenshots-store release (ID={ReleaseId})", created.Id);
                return created.UploadUrl;
            }
            catch (Exception ex) when (ex.Message.Contains("already_exists", StringComparison.OrdinalIgnoreCase))
            {
                // Race: another instance created it — re-fetch
                var existing = await _client.Repository.Release.Get(_owner, _repo, ScreenshotReleaseTag);
                _screenshotReleaseId = existing.Id;
                _screenshotUploadUrl = existing.UploadUrl;
                return existing.UploadUrl;
