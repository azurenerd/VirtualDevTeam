...
public class ConflictResolver
{
        ...
        ILogger<ConflictResolver> logger)
    {
        ...
        if (repoParts.Length == 2 && !string.IsNullOrWhiteSpace(repoParts[0]))
        {
            _owner = repoParts[0];
            _repo = repoParts[1];
        }
        ...
        var token = projectConfig.GitHubToken;
        if (!string.IsNullOrWhiteSpace(token))
        {
            _client = new GitHubClient(new ProductHeaderValue("VirtualDevTeam-ConflictResolver"))
            {
        ...
        {
            // Deferred initialization — client will be set via Reconfigure() when token becomes available
            _client = new GitHubClient(new ProductHeaderValue("VirtualDevTeam-ConflictResolver"));
            _logger.LogWarning("ConflictResolver initialized without a GitHub token. Conflict detection will fail until Reconfigure() is called or a token is provided.");
        }
    }
    ...
    public void Reconfigure(string owner, string repo, string token)
    {
        _owner = owner;
        _repo = repo;
        var client = new GitHubClient(new ProductHeaderValue("VirtualDevTeam-ConflictResolver"));
        if (!string.IsNullOrWhiteSpace(token))
        {
