...
public sealed class StrategyTaskStepBridge : IStrategyEventSink
{
    ...
    public void UnregisterTask(string runId, string taskId, bool succeeded, string? winnerStrategy = null)
    {
        ...
        // Complete any leaked child steps (eval step, still-InProgress candidates)
        if (reg.EvalStepId is not null)
        {
            try { _tracker.CompleteStep(reg.EvalStepId); } catch { }
        }
        foreach (var (_, stepId) in reg.CandidateStepIds)
        {
            try { _tracker.CompleteStep(stepId, AgentTaskStepStatus.Skipped); } catch { }
        }
    ...
    private void OnEvaluationProgress(EvaluationProgressEvent e)
    {
        if (!_registrations.TryGetValue((e.RunId, e.TaskId), out var reg)) return;

        // Create an evaluation step on the first progress event; update on subsequent ones
        if (reg.EvalStepId is null)
        {
            reg.EvalStepId = _tracker.BeginChildStep(reg.AgentId, reg.TaskId, reg.ContainerStepId,
                "⚖️ Evaluating gates and scoring", e.Detail ?? e.Phase);
        }
        else
        {
            _tracker.RecordSubStep(reg.EvalStepId, e.Detail ?? e.Phase);
        }
    }
    ...
    private void OnWinnerSelected(WinnerSelectedEvent e)
    {
        ...
        // Complete the evaluation step if one was created
        if (reg.EvalStepId is not null)
            _tracker.CompleteStep(reg.EvalStepId);
    ...
    private sealed class TaskRegistration
    {
        ...
        public string? EvalStepId { get; set; }
