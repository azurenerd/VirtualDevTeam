...
public static partial class ConfigValidator
{
    ...
    public static List<string> Validate(VirtualDevTeamConfig config)
    {
        var errors = new List<string>();

        ValidateProject(config.Project, config.DevPlatform.AuthMethod, errors);
    ...
    private static void ValidateProject(ProjectConfig project, DevPlatform.Config.DevPlatformAuthMethod authMethod, List<string> errors)
    {
        ...
        // GitHubToken is not required when using GitHub CLI auth (GhCli) — token comes from `gh auth token`
        if (!string.IsNullOrWhiteSpace(project.GitHubRepo)
            && string.IsNullOrWhiteSpace(project.GitHubToken)
            && authMethod != DevPlatform.Config.DevPlatformAuthMethod.GhCli)
        {
