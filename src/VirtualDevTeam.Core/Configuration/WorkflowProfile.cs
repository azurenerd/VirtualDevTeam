...
public interface IWorkflowProfile
{
    ...
    /// Empty string for project mode (repo root), ".virtualdevteam/features/{id}" for feature mode.
    /// </summary>
...
{
    private readonly PrDeliveryMode _prMode;
    private readonly string _docsFolderPath;
    private readonly string _runScope;

    public ProjectWorkflowProfile(PrDeliveryMode prMode, string docsFolderPath = "AgentDocs", string? runScope = null)
    {
        _prMode = prMode;
    ...
    [Obsolete("Use the PrDeliveryMode constructor.")]
    public ProjectWorkflowProfile(bool singlePrMode = true, string docsFolderPath = "AgentDocs", string? runScope = null)
        : this(singlePrMode ? PrDeliveryMode.SinglePR : PrDeliveryMode.MultiPRParallel, docsFolderPath, runScope) { }
    ...
    {
        "ProgramManager", "Researcher", "Architect", "SoftwareEngineer", "TestEngineer", "SecurityAuditor"
    };
    ...
    public bool DecomposeToMultipleTasks => _prMode is PrDeliveryMode.MultiPRParallel or PrDeliveryMode.MultiPRSerial;

    /// <summary>The PR delivery mode for this workflow run.</summary>
    public PrDeliveryMode PrMode => _prMode;
...
public class FeatureWorkflowProfile : IWorkflowProfile
{
    ...
    public string ArtifactBasePath => $"AgentDocs/features/{_feature.Id}";
