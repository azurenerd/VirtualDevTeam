...
public static partial class ConfigWizard
{
    ...
    public static VirtualDevTeamConfig Run()
    {
        var config = new VirtualDevTeamConfig();

        WriteHeader("VirtualDevTeam Configuration Wizard");
        Console.WriteLine();
        ...
        if (PromptYesNo("Configure Anthropic (Claude)?", true))
        {
            ...
            {
                Provider = "anthropic",
                Model = Prompt("Premium model name", "claude-opus-4.8"),
                ApiKey = apiKey,
    ...
    public static async Task SaveToFileAsync(VirtualDevTeamConfig config, string filePath)
    {
    ...
    public static string SerializeToJson(VirtualDevTeamConfig config)
    {
        return JsonSerializer.Serialize(config, JsonOptions);
    }

    private static void AssignAgentModelTiers(VirtualDevTeamConfig config)
    {
