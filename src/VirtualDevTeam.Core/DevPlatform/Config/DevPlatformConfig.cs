namespace VirtualDevTeam.Core.DevPlatform.Config;
...
public enum DevPlatformType
{
    GitHub,
    AzureDevOps,
    /// <summary>Local bare repo + SQLite. Agents work locally; one clean PR submitted to upstream at completion.</summary>
    Local
}
...
public enum DevPlatformAuthMethod
{
    ...
    /// <summary>GitHub CLI token (GitHub only). Uses gh auth token.</summary>
    GhCli
}
...
public class DevPlatformConfig
{
    ...
    /// <summary>Authentication method. Default: GhCli (uses gh auth token).</summary>
    public DevPlatformAuthMethod AuthMethod { get; set; } = DevPlatformAuthMethod.GhCli;
    ...
    /// <summary>
    /// Configurable work item state mappings from VirtualDevTeam internal states to platform states.
    /// Keys are VirtualDevTeam states: "Open", "InProgress", "Blocked", "Resolved".
...
public class AzureDevOpsConfig
{
    ...
    /// Store via user secrets: dotnet user-secrets set "VirtualDevTeam:DevPlatform:AzureDevOps:BearerToken" "token"
    ...
    /// <summary>
    /// Configurable work item state mappings from VirtualDevTeam internal states to ADO states.
