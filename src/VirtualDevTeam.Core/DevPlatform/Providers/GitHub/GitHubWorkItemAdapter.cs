...
public sealed class GitHubWorkItemAdapter : IWorkItemService
{
    ...
    public GitHubWorkItemAdapter(IGitHubService github, IOptions<VirtualDevTeamConfig> config)
    {
    ...
    {
        // Use the filtered call (run-scoped) — NOT GetAllIssuesUnfilteredAsync which returns
        // all issues across all runs (66+ items) and causes timeline bloat.
        var issues = await _github.GetAllIssuesAsync(ct);
        return issues.Select(GitHubModelMapper.ToPlatform).ToList();
    }
