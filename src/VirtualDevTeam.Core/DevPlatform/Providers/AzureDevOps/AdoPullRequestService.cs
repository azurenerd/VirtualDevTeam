...
public sealed class AdoPullRequestService : AdoHttpClientBase, IPullRequestService
{
    ...
    // Multiple agents poll list endpoints every few seconds; caching collapses concurrent reads.
    ...
    {
        var cacheKey = $"Get|{id}";
        if (TryGetCached<PlatformPullRequest>(cacheKey, out var cached)) return cached;
        ...
        var result = AdoModelMapper.ToPlatform(pr, Organization, Project, Repository);
        SetCached(cacheKey, result, GetByNumberTtl);
        return result;
    }
    ...
    public async Task<IReadOnlyList<PlatformPullRequest>> ListOpenAsync(CancellationToken ct = default)
    {
        ...
        if (TryGetCached<IReadOnlyList<PlatformPullRequest>>(cacheKey, out var cached)) return cached!;
        ...
        SetCached(cacheKey, (IReadOnlyList<PlatformPullRequest>)results, ListOpenTtl);
        return results;
    }
    ...
    public async Task<IReadOnlyList<PlatformPullRequest>> ListAllAsync(CancellationToken ct = default)
    {
        ...
        // ADO list endpoint excludes labels. Hydrate labels for lifecycle/dashboard display.
        // Bounded to $top=200; per-PR label call is acceptable for dashboard refresh cadence.
        var results = new List<PlatformPullRequest>();
        foreach (var listPr in adoPrs)
        {
            var hydrated = await HydrateLabelsAsync(listPr, ct);
            results.Add(AdoModelMapper.ToPlatform(hydrated, Organization, Project, Repository));
        }
    ...
    // ADO has no run-scope filter on ListAllAsync, so this is a direct delegate.
    ...
    {
        const string cacheKey = "ListMerged";
        if (TryGetCached<IReadOnlyList<PlatformPullRequest>>(cacheKey, out var cached)) return cached!;
        ...
        SetCached(cacheKey, (IReadOnlyList<PlatformPullRequest>)prs, ListOpenTtl);
        return prs;
    }
