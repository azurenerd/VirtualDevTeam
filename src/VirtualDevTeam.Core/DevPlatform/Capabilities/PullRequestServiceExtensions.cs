...
public static class PullRequestServiceExtensions
{
    ...
    /// <summary>
    /// Atomically remove a label from a PR by re-fetching current labels immediately
    /// before writing. No-op when the PR doesn't have the label or doesn't exist.
    /// </summary>
    public static async Task RemoveLabelAsync(
        this IPullRequestService prService, int prId, string label, CancellationToken ct = default)
    {
        var freshPr = await prService.GetAsync(prId, ct);
        if (freshPr is null) return;
