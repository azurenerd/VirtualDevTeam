...
public partial class SoftwareEngineerAgent
{
        ...
        AgentPullRequest pr, CancellationToken ct)
    {
        try
        {
            ...
            var totalPromptSize = reviewContextBuilder.Length + (systemPrompt?.Length ?? 0);
            if (totalPromptSize > 100_000)
                Logger.LogWarning("SE peer review prompt for PR #{PrNumber} is {Size:N0} chars — consider CLI review mode for large PRs",
        ...
        List<EngineeringTask> tasks, List<EngineerInfo> engineers, CancellationToken ct)
    {
        ...
        try
        {
            ...
            prompt.AppendLine("7. ⚠️ NARROW-DOMAIN SPECIALIST RULE: When an engineer's capabilities describe a narrow " +
                              "delivery domain (visual assets, security audits, database migrations, ML pipelines, " +
                              "compliance reviews, technical writing, accessibility audits, etc.), do NOT assign them " +
                              "tasks whose deliverables fall outside that domain — even if a few incidental keywords " +
                              "overlap. Pick the engineer whose capability set has the STRONGEST semantic match with " +
                              "the task's primary deliverable; let other engineers be unassigned. It is correct for a " +
                              "narrow-domain specialist to stay idle when there's no in-domain work, and incorrect to " +
                              "force-fit them onto unrelated tasks they cannot perform well.");
            prompt.AppendLine();
            ...
            // Build validated assignment map (last-wins for safety against duplicate task IDs)
            var taskLookup = new Dictionary<string, EngineeringTask>(StringComparer.OrdinalIgnoreCase);
