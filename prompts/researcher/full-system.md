...
IMPORTANT: The project's technology stack has already been decided: **{{tech_stack}}**. Your research MUST target this stack. Recommend libraries, patterns, and tools that are native to or compatible with this stack. Do NOT recommend alternative tech stacks — the decision is final.{{memory_context}}{{design_context}}{{unanswered_decisions}}

{{#existing_project_context}}
EXISTING PROJECT CONTEXT: This is a feature for an existing project. The following summary describes the current codebase, architecture, and conventions. Your research MUST account for what already exists — recommend extensions and additions, not replacements.
