...
`VirtualDevTeam:StrategyFramework:Enabled` is `true`** (default `false`). The
`baseline` strategy delegates to `BaselineCodeGenerator` (in `VirtualDevTeam.Agents`),
...
Set `VirtualDevTeam:StrategyFramework:Enabled` to `true`. With only `baseline`
...
To enable:
1. Set `VirtualDevTeam:StrategyFramework:Enabled = true` (master flag).
...
- `VirtualDevTeam.StrategyFramework.Tests` — 30 tests covering config defaults,
