...
- `docs/VirtualDevTeamGapAnalysis.md` (the other CLI's root-cause write-up — owns remediations R1–R7, of which R1/R2/R3 have shipped in commit `e99ff88`).
...
PR #2136 (T4 "Implement Global Page Layout, CSS, and Dashboard Shell") was merged with:
...
**Still unshipped and owned by `docs/VirtualDevTeamGapAnalysis.md`:**
...
- ⏳ R7 — Golden-case end-to-end test of VirtualDevTeam itself. **Convergent with my WS3.** Suggested merge: my WS3 (offline harness + fixture format) becomes the infrastructure; R7's golden-case test becomes the first fixture.
...
- `VirtualDevTeamGapAnalysis.md` owns R4, R5, R6, R7 (strategic reshapes).
...
- File: `src/VirtualDevTeam.Agents/EngineerAgentBase.cs` rework path (and equivalent Architect/PM/TE paths).
...
- File: `src/VirtualDevTeam.Agents/ProgramManagerAgent.cs` (~line 960 where `hasTeCompletionComment` is built).
...
- File: `src/VirtualDevTeam.Agents/EngineerAgentBase.cs` line ~1500 (`HandleReworkAsync` force-approval path).
- New config flag: `VirtualDevTeam.Gates.ReworkExhaustion.BlockOnUiFailures = true` (default true).
...
- File: `src/VirtualDevTeam.Agents/ProgramManagerAgent.cs` (PM screenshot-review prompt builder, lines 2556–2600).
...
- File: `src/VirtualDevTeam.Agents/TestEngineerAgent.cs` line ~1720 (`UploadTestArtifactsToPrAsync`).
...
- File: `src/VirtualDevTeam.Agents/TestEngineerAgent.cs` line ~1599 (standalone capture fallback).
...
- File: `src/VirtualDevTeam.Agents/TestEngineerAgent.cs` (before the Playwright runner step).
...
- File: `src/VirtualDevTeam.Agents/SoftwareEngineerAgent.cs` (ready-for-review branch, near existing self-review step).
...
- File: `src/VirtualDevTeam.Agents/SoftwareEngineerAgent.cs` — scaffolding/foundation task handling (T1-class tasks).
...
- R2-1: If hypothesis 5 (own-PR downgrade) is root cause — the comments ARE in the downgraded body but as a text block, not on lines. The only fix is a separate `github-actions[bot]`-equivalent reviewer account OR accepting the conversation-tab format is a fundamental limitation of the single-PAT setup. Options:
  ...
  - (b) Add a GitHub App / bot account for reviews and store its token alongside the main PAT (`VirtualDevTeam.Review.BotToken`). Use bot token exclusively in `SubmitReviewAsync`.
...
- R2-3: If hypothesis 3 (config toggled off) — flip on in `appsettings.json` and set default in `VirtualDevTeamConfig.cs` to `EnableInlineComments = true`.
...
- `tests/VirtualDevTeam.Integration.Tests/` — already hosts workflow-level tests (`WorkflowStateMachineTests`, `SystemBootstrapTests`).
- `tests/VirtualDevTeam.FakeCopilotCli/` — already a fake CLI process; currently minimal.
- No fake `IGitHubService` exists yet — all agents talk directly to real `GitHubService`. This is the central gap.

### Design

#### Component 1 — `InMemoryGitHubService : IGitHubService` (new, in `tests/VirtualDevTeam.Integration.Tests/Fakes/`)
...
- A test fixture is a JSON file describing a single PR lifecycle replay:
  ...
  ```
- Store under `tests/VirtualDevTeam.Integration.Tests/Fixtures/`.
