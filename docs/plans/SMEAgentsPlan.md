...
> **Purpose:** Comprehensive plan for enabling VirtualDevTeam agents to dynamically create new specialized agents ("SME Agents") with configurable MCP server toolsets, external knowledge sources, and custom role definitions. This plan builds on the existing VirtualDevTeam architecture and extends it to support runtime agent composition.
>
...
Enable VirtualDevTeam agents (primarily the PM and SE) to **dynamically define and spawn new agents at runtime** with:
...
**`AgentCallContext.McpServers`** (`src/VirtualDevTeam.Core/AI/AgentCallContext.cs`):
```csharp
...
**`CopilotCliProcessManager`** (`src/VirtualDevTeam.Core/AI/CopilotCliProcessManager.cs`, line ~293):
```csharp
...
**`AgentConfig.McpServers`** (`src/VirtualDevTeam.Core/Configuration/VirtualDevTeamConfig.cs`, line ~100):
```csharp
...
**`RoleContextProvider`** (`src/VirtualDevTeam.Core/AI/RoleContextProvider.cs`):
...
**`AgentConfig`** in `VirtualDevTeamConfig.cs`:
```csharp
...
**`AgentSpawnManager`** (`src/VirtualDevTeam.Orchestrator/AgentSpawnManager.cs`):
...
**`AgentFactory`** (`src/VirtualDevTeam.Agents/AgentFactory.cs`):
...
MCP servers need to be **runnable** on the host machine. There are three approaches, and the right choice depends on how VirtualDevTeam's Copilot CLI integration works:
...
public class CopilotCliMcpConfigManager
{
    ...
    /// Removes servers that were added by VirtualDevTeam (tracks via metadata)
    ...
    {
        // Auto-inject known secrets from VirtualDevTeam config
...
| Secret leakage | Env vars for MCP servers pulled from VirtualDevTeam config (not hardcoded in definitions); definitions stored without secrets |
...
- Add `McpServerDefinition` record to `VirtualDevTeam.Core`
- Add `McpServersConfig` to `VirtualDevTeamConfig`
...
- Add `SmeAgentsConfig` to `VirtualDevTeamConfig`
...
- Create `CopilotCliMcpConfigManager`
  ...
  - Secret injection from VirtualDevTeam config
  - Cleanup on shutdown
