...
- `src/VirtualDevTeam.Core/Configuration/VirtualDevTeamConfig.cs` line 211, 218, 225, 231 — all four rework limits default to **1**.
- `src/VirtualDevTeam.Runner/appsettings.json` line 118 reinforces `MaxReworkCycles: 1`.
...
- A scaffolding-phase PR should have shipped at minimum a `data.template.json` (already exists at repo root in VirtualDevTeam's own workspace — user even maintains one) into the target app's `wwwroot/` so the app boots non-blank from day 1.
...
- **A1** Raise defaults in `VirtualDevTeamConfig.cs`: `MaxReworkCycles = 3`, `MaxPmReworkCycles = 3`, `MaxArchitectReworkCycles = 2`, `MaxTestReworkCycles = 2`. Also raise in `appsettings.json`.
