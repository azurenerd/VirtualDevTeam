...
Complete guide to installing, configuring, and running VirtualDevTeam.
...
VirtualDevTeam requires a way to authenticate with your dev platform (GitHub or Azure DevOps). For AI providers, the default **Copilot CLI provider** routes all model tiers through the GitHub Copilot CLI binary — no API keys needed. If you prefer direct API access (or as a fallback), configure individual provider keys below.

### GitHub Authentication

VirtualDevTeam supports three authentication methods for GitHub:

#### Option 1: GitHub CLI Auth (Recommended — Default)

The simplest setup — uses `gh auth token` to dynamically resolve your token. Ideal for EMU (Enterprise Managed User) accounts.

1. Install the [GitHub CLI](https://cli.github.com/)
2. Log in: `gh auth login`
3. Verify: `gh auth status`

No token is stored in VirtualDevTeam config — it's resolved dynamically from the GitHub CLI on each use.
...
```bash
# Recommended: Good balance of speed and quality for coding tasks
ollama pull qwen2.5-coder:14b

# Lighter alternative (configured as default in appsettings.json)
...
All configuration is in `src/VirtualDevTeam.Runner/appsettings.json` under the `VirtualDevTeam` section.
...
Start the dashboard standalone (`cd src/VirtualDevTeam.Dashboard && dotnet run`) and navigate to `/develop`.
...
1. **Bootstrap** — The `VirtualDevTeamWorker` spawns the 5 core agents in order:
...
- **Console output** shows ASCII banner and `[VirtualDevTeamWorker] Core agents spawned successfully`
...
**Solutions:**
- The database file (`virtualdevteam.db`) is created automatically in the working directory
- Ensure the process has write permissions to the current directory
- Delete `virtualdevteam.db` to reset state (agents will re-initialize from scratch)
