...
This guide explains how to configure VirtualDevTeam to use Azure DevOps (ADO) as the underlying platform for pull requests, work items, code hosting, and reviews.
...
# Token is auto-refreshed by VirtualDevTeam — no manual steps after login
```

VirtualDevTeam uses the ADO resource ID (`499b84ac-1321-427f-aa17-267ca6975798`) and auto-refreshes tokens 5 minutes before expiry.
...
| VirtualDevTeam Concept | GitHub | Azure DevOps |
|---|---|---|
...
ADO uses customizable states per work item type. The `StateMappings` config maps VirtualDevTeam's internal states to your ADO process template states:

| VirtualDevTeam State | Default ADO State | Description |
|---|---|---|
...
2. **Work item hierarchy** — ADO supports parent/child relationships natively. VirtualDevTeam leverages this for task decomposition.
3. **WIQL queries** — Work item filtering uses ADO's SQL-like query language internally.
4. **Rate limiting** — ADO uses `X-RateLimit-*` headers. VirtualDevTeam tracks these and backs off automatically.
...
7. **PR description limit** — ADO enforces a hard 4000-character limit on PR descriptions. VirtualDevTeam auto-truncates with a suffix note.
8. **HTML descriptions** — ADO Work Item Description field renders HTML, not Markdown. VirtualDevTeam auto-converts Markdown to HTML via Markdig before writing.
...
Agent workspaces (C:\Agents4\) must clone from the ADO repository URL, not GitHub. The `GetGitCloneUrl()` helper on `VirtualDevTeamConfig` returns the correct URL based on `DevPlatform.Platform`.
...
3. Delete state DB (`virtualdevteam.db`)
...
When `Platform = AzureDevOps`, the DI container registers ADO implementations for each interface. When `Platform = GitHub`, it registers GitHub adapter classes that wrap the existing `IGitHubService`. The rest of VirtualDevTeam code works against the interfaces — no platform-specific logic leaks into agents, orchestrator, or dashboard.
