...
- **Stale DLL copies**: `dotnet build src/VirtualDevTeam.Agents/...` does **not** refresh `src/VirtualDevTeam.Runner/bin/Debug/net8.0/VirtualDevTeam.Agents.dll`. Always build the Runner project (or the whole solution) so referenced DLLs are copied into the host's bin. Symptom: code changes appear to do nothing at runtime.
...
- **`src/VirtualDevTeam.Core/Strategies/McpEnhancedStrategy.cs`** (id=`mcp-enhanced`). Delegates
...
- **`src/VirtualDevTeam.Core/Mcp/IMcpServerLocator.cs`** — abstraction + `DefaultMcpServerLocator`.
  Resolution order: `StrategyFrameworkConfig.McpServerDllPath` (explicit), then
  `AppContext.BaseDirectory/VirtualDevTeam.McpServer.dll` (production shape, alongside the host),
  then dev-tree probe via repo-root + `src/VirtualDevTeam.McpServer/bin/<cfg>/net8.0/...`.
...
- **`src/VirtualDevTeam.Runner/VirtualDevTeam.Runner.csproj`** — project reference + explicit
  `Copy` target that drops `VirtualDevTeam.McpServer.dll` + `.runtimeconfig.json` + `.deps.json`
...
Test additions (`tests/VirtualDevTeam.StrategyFramework.Tests/`):
...
Landed in 2a:
- `src/VirtualDevTeam.McpServer/` — standalone stdio JSON-RPC server (init, tools/list,
...
- `src/VirtualDevTeam.Core/Mcp/McpConfigWriter.cs` — pure utility. `BuildConfig` + atomic
...
INSERT INTO todos (id, title, status) VALUES
 ...
 ('p0-tests-project','Add VirtualDevTeam.StrategyFramework.Tests xUnit project','done'),
