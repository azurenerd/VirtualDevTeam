# VirtualDevTeam Requirements & Workflow Scenarios

> **Purpose:** This document captures all requirements for the VirtualDevTeam multi-agent AI developer team system. Each requirement includes expected workflow scenarios that can be used by testing agents, developers, or new Copilot sessions to validate the system operates correctly and to create/fix bugs when it doesn't.
>
...
2. `VirtualDevTeamWorker` spawns core agents in phase order: PM → Researcher → Architect → SE
...
5. `VirtualDevTeamWorker` spawns enabled custom agents from configuration
...
- **REQ-ROLE-003b**: `CustomAgentConfig` extends `AgentConfig` with `Name` and `Enabled` properties. Custom agents are listed under `VirtualDevTeam:CustomAgents` in configuration.
...
- **REQ-ROLE-003d**: `AgentFactory` can create custom agents. `VirtualDevTeamWorker` spawns all enabled custom agents at startup after core agents are initialized.
...
2. On startup, `VirtualDevTeamWorker` reads custom agent configs → finds SecurityAuditor with Enabled=true
...
- **REQ-SE-002l**: **Visual Verification section in engineering task issues.** Every engineering task issue MUST include a `## Visual Verification` section specifying: (1) app type (`web-ui`, `api-only`, `cli`, or `library`), (2) test URL paths relative to the app root (e.g., `/`, `/login`, `/api/health`), and (3) expected visual results for each URL. This section drives the media capture pipeline's URL targeting — `PlaywrightRunner.ExtractTestUrlPaths()` parses it to determine which pages to screenshot/record. Tasks for `api-only`, `cli`, or `library` app types specify no test URLs and skip visual capture. *(Added 2026-05-15)*
...
- **REQ-GH-001c**: Code PRs include a `.virtualdevteam/{task}.task` marker file so the branch differs from main before any code is committed.
...
### REQ-REV-009: Duplicate Review Comment Prevention

- **REQ-REV-009a**: When inline review comments are submitted via `CreateReviewWithInlineCommentsAsync` (using the GitHub Reviews API with `COMMENT` event type per REQ-REV-008), the standalone summary comment that would normally be posted by the reviewer agent MUST be suppressed. The inline review already contains the full review body as its top-level comment — posting a separate summary comment duplicates the content on the PR.
- **REQ-REV-009b**: The suppression check MUST verify that inline comments were actually submitted successfully before skipping the summary. If `CreateReviewWithInlineCommentsAsync` fails or falls back to no-inline mode, the standalone summary comment MUST still be posted as the fallback communication path.
- **REQ-REV-009c**: This applies to all reviewer roles (Architect, PM, SE peer review) that use the inline review path. *(Added 2026-05-15)*
...
- **REQ-CFG-003b**: All config under the `VirtualDevTeam` section, bound via `IOptions<VirtualDevTeamConfig>`.
...
- **REQ-CFG-004a**: `WorkspaceConfig` section under `VirtualDevTeam:Workspace`. Controls local build/test infrastructure for all agents.
...
- **REQ-CFG-007d**: `AgentConfig` model has `RoleDescription`, `McpServers`, and `KnowledgeLinks` properties. These are configured per-agent under `VirtualDevTeam:Agents:{Role}` in appsettings.json.
...
- **REQ-CFG-008a**: Global MCP server definitions are configured under `VirtualDevTeam:McpServers` in appsettings.json. Each server is defined by `McpServerDefinition` record: Name, Description, Command, Args, Env, Transport (Stdio/Http/Sse), RequiredRuntimes, ProvidedCapabilities.
...
- **REQ-CFG-009a**: SME system configuration lives under `VirtualDevTeam:SmeAgents` section, bound to `SmeAgentsConfig`. Key properties: `Enabled` (default true), `MaxTotalSmeAgents` (default 5), `AllowAgentCreatedDefinitions` (default true), `PersistDefinitions` (default true), `DefinitionsPath` (default `sme-definitions.json`).
...
- **REQ-IDEM-004j**: `AgentSpawnManager.RespawnAgentAsync(agentId)` MUST support restarting a single agent in place without a full runner restart. The replacement agent keeps the same identity/display name, re-registers with `AgentRegistry`, and resumes against durable platform state (PRs, issues, reviews, memory) so operators can recover an individual stuck agent without resetting the whole run.

### REQ-IDEM-005: Runner Startup vs Fresh Reset (Task File Preservation)

- **REQ-IDEM-005a**: **Runner startup MUST NOT delete task files** (`.virtualdevteam/*.task` marker files, `TeamMembers.md`, local workspace directories). These must persist so the runner can resume work after an accidental exit, computer restart, or crash. The runner startup path is the "resume" path.
- **REQ-IDEM-005b**: **Fresh reset (via dashboard or CLI script) MUST delete everything** needed for a clean start: all GitHub PRs (closed), all GitHub Issues (closed with appropriate labels), all non-default branches deleted, all repo files removed, the SQLite state database, and local workspace directories.
...
- **REQ-DASH-001a**: Dashboard can run in two modes: (1) embedded in the Runner process (in-process, default), or (2) as a standalone process via `VirtualDevTeam.Dashboard.Host` on port 5051, separate from the Runner on port 5050. Standalone mode allows UI changes/restarts without killing running agents.
...
- **REQ-DASH-002g**: Agent cards MUST expose a per-agent restart control so operators can restart only the selected agent when it is stuck, without restarting the full runner.
- **REQ-DASH-002h**: The per-agent restart action MUST call the Runner REST endpoint `POST /api/dashboard/agents/{agentId}/restart`, which delegates to `AgentSpawnManager.RespawnAgentAsync` and returns success/failure for the targeted agent only.
...
- **REQ-DASH-006a**: Dashboard port configurable via `VirtualDevTeam:Dashboard:Port` (default: 5050). Value wired to Kestrel at startup via `builder.WebHost.UseUrls()`.
...
- **REQ-DASH-008a**: `VirtualDevTeam.Dashboard.Host` is a standalone Blazor Server project that runs on port 5051, separate from the Runner on port 5050. UI changes/restarts do not affect running agents.
...
### REQ-DASH-007: Pipeline Status Snapshot API

- **REQ-DASH-007a**: Runner MUST expose `GET /api/pipeline/status` as a single-call pipeline snapshot for CLI monitoring, FlowMonitor, and external tools.
- **REQ-DASH-007b**: The snapshot MUST consolidate the current workflow phase, live agent state, open work items, linked PR lifecycle data, dependency relationships, and summary metrics so operators do not need to query multiple endpoints to understand pipeline state.
- **REQ-DASH-007c**: Platform fetch failures MUST degrade gracefully to partial data with warning logging instead of failing the entire endpoint response.
...
- **REQ-DASH-013d**: Running candidates on `/strategies` MUST expose a per-candidate `🔄` reset action with two-click confirmation (`🔄` → `⚠ Reset?`). Confirming reset SHALL call `POST /api/strategies/reset/{runId}/{taskId}/{strategyId}` and request a fresh retry from scratch.
...
- **REQ-DASH-014c**: Dashboard action buttons with destructive or recovery semantics MUST explain their impact via tooltips. This includes strategy **Reset** (kill process + fresh-worktree retry + escalation ladder), strategy **Cancel** (permanent stop, no retry), **Cancel All** (abort entire task/orchestration), and agent **Restart** (drops in-flight LLM work but recovers durable PR/issue state).

### REQ-DASH-015: Frameworks Nav Badge

- **REQ-DASH-015a**: The "Frameworks" navigation menu item MUST display a real-time count badge showing the number of currently active strategy orchestrations. The badge renders as an inline pill (e.g., `Frameworks (2)`) when ≥1 orchestration is running.
- **REQ-DASH-015b**: The badge MUST disappear (no `(0)` shown) when no strategy orchestrations are active — idle state shows the bare "Frameworks" label.
- **REQ-DASH-015c**: Badge count is sourced from `StrategyOrchestrator` active-run tracking and updated via SignalR push (in-process dashboard) or REST polling (standalone dashboard). *(Added 2026-05-15)*
...
- **REQ-AI-004a**: The default **premium** tier model is `claude-opus-4.7`. All default config (`appsettings.json`, `VirtualDevTeamConfig`, `ConfigWizard`, the dashboard `Configuration.razor`, and `copilot-instructions.md`) MUST reference `claude-opus-4.7` as the premium default, not a prior Opus version.
...
### REQ-AI-005: FreshPathResolver for Child Process Spawning

- **REQ-AI-005a**: All child process spawns that use tools potentially installed at runtime (`ffmpeg`, `npm`, `copilot`, `squad`, `az`, `gh`) SHOULD use `FreshPathResolver` (`Core/AI/FreshPathResolver.cs`) to resolve the executable path. `FreshPathResolver` reads the current Machine + User PATH from the Windows registry instead of relying on the inherited process PATH, which is stale if tools were installed after the runner started (e.g., `ffmpeg` installed via winget during the Welcome wizard).
- **REQ-AI-005b**: `FreshPathResolver.Resolve(executableName)` returns the full path to the executable if found on the fresh registry PATH, or `null` if not found. Callers SHOULD fall back to the bare executable name (inherited PATH) when `Resolve` returns null.
- **REQ-AI-005c**: Currently applied in `GifConverter` and `VideoTrimmer`. SHOULD be progressively adopted by `CopilotCliProcessManager`, `SquadFrameworkAdapter`, and any new child process launcher. *(Added 2026-05-15)*
...
- **REQ-INTEG-001f**: **T-FINAL dependency merge verification.** `CreateIntegrationPRAsync` MUST NOT start the T-FINAL integration task until ALL dependency engineering PRs are merged to the base branch. Before creating the integration branch, the SE checks for open engineering PRs authored by agents in this project. If any remain open (not yet merged), T-FINAL defers and retries on the next poll cycle. This prevents merge conflicts from unmerged dependency branches being included in the integration PR (previously caused 22-file conflicts when a dependency PR hadn't merged yet — see Lesson #35 in copilot-instructions.md). *(Added 2026-05-15)*
...
### REQ-MTEST-004: Health Probe Before Media Capture

- **REQ-MTEST-004a**: Both MCP Playwright exploration and DirectCapture MUST verify the target application is alive with a 5-second HTTP health probe (GET to the app's root URL) before starting any navigation, screenshot capture, or video recording. A non-responsive server is logged at Warning level and the capture attempt is skipped — no further Playwright operations are attempted for that target.
- **REQ-MTEST-004b**: The health probe prevents wasting 2+ minutes on Playwright navigation timeouts against dead servers. Without the probe, failed capture attempts consumed full browser launch → navigate → timeout cycles before reporting failure.
- **REQ-MTEST-004c**: Health probe failure MUST produce clear diagnostic logging including the target URL, response status (or timeout), and the reason the capture was skipped, so operators can distinguish "app not running" from "Playwright broken" in logs.
...
- **REQ-GATE-002a**: `GateCheckService` uses `IOptionsMonitor<VirtualDevTeamConfig>` (not `IOptions`) so gate configuration changes in appsettings.json are picked up at runtime without requiring a runner restart.
...
- **REQ-MCP-001b**: MCP server definitions are configured globally under `VirtualDevTeam:McpServers` in appsettings.json. All agents reference servers by name — definitions are shared, not duplicated per-agent.
...
### REQ-MCP-005: MCP Exploration Acceptance Criteria Targeting

- **REQ-MCP-005a**: The MCP Playwright exploration agent MUST navigate to acceptance criteria test URLs FIRST, before performing general page exploration. Test URLs are extracted from the engineering task issue description via `PlaywrightRunner.ExtractTestUrlPaths()` and injected at the top of the MCP exploration prompt as a structured data block.
- **REQ-MCP-005b**: The MCP prompt MUST be structured with data (test URLs) BEFORE instructional steps. The agent reads top-down — placing "navigate to URLs listed above" in the steps while the URLs appear below causes the agent to miss them entirely. Correct order: data block → steps → rules. *(Lesson #32 in copilot-instructions.md)*
- **REQ-MCP-005c**: If no test URLs are extracted (e.g., `api-only` app type or missing `## Visual Verification` section), the MCP exploration agent falls back to general root-URL exploration. This is graceful degradation, not an error.
...
### REQ-PROMPT-004: No Truncation of Design Documents in Prompts

- **REQ-PROMPT-004a**: Agent prompts MUST pass full PMSpec.md and Architecture.md documents without truncation when injecting them as template variables (`{{pm_spec}}`, `{{architecture}}`). With 1M context window models (e.g., `claude-opus-4.7`), hard-cutting documents at arbitrary character limits (previously 3000 chars) loses important context — user stories, acceptance criteria, architecture decisions, and component specifications in the latter portions of these documents are silently dropped.
- **REQ-PROMPT-004b**: If a token budget constraint requires limiting document size, the truncation limit MUST be configurable (not hardcoded) and MUST log a Warning when truncation occurs, including the original size and truncated size, so operators are aware context was lost.
- **REQ-PROMPT-004c**: This applies to all agents that consume PMSpec.md or Architecture.md: Architect, Software Engineer, Specialist Engineers, Test Engineer, and PM (for cross-reference during reviews). *(Added 2026-05-15)*
...
- **REQ-SCOPE-004d**: Run-scoped PR membership checks MUST delegate to the shared `PullRequestWorkflow.IsCurrentRunScopePr(headBranch, prBody, runScope)` utility. Agent-specific code SHOULD NOT duplicate inline branch/body/run-scope filtering logic.
...
> **Status (2026-04-19):** Phases 0–6 shipped, feature-flagged via `VirtualDevTeam:StrategyFramework:Enabled` (default **OFF**). `val-e2e` live-pipeline validation complete (run `20260419T231321Z`, task T-1926: baseline + mcp-enhanced both ran cleanly, winner=baseline via llm-rank, winner patch applied to PR). See `docs/StrategyFramework.md` for the authoritative status table.

### REQ-STRAT-001: Master Feature Flag

**REQ-STRAT-001a**: The Strategy Framework MUST default to OFF (`VirtualDevTeam:StrategyFramework:Enabled=false`) so existing single-pass SE behaviour is preserved.
...
**REQ-STRAT-002e**: Candidate runtime state MUST record the spawned process ID and process-start timestamp in `CandidateStateStore`. This state enables detection and management of stuck strategy candidate processes without losing per-candidate isolation.

**REQ-STRAT-002f**: Operator-requested candidate resets MUST cancel the current candidate CTS/process via `OrchestrationCancellationService.RequestCandidateReset`, mark the candidate for reset, then retry in a fresh worktree with a fresh linked CTS. Reset retries use the same escalation ladder as transient stuck recovery: rung 1 repeats the same configuration, rung 2 retries with `ForceNoWrapper=true`.
...
```
1. Set VirtualDevTeam:StrategyFramework:Enabled=true in appsettings.Development.json
...
### REQ-STRAT-013: Scenario Approval Persistence

**REQ-STRAT-013a**: All scenario status changes (Approve, Reject, Edit) MUST persist to `develop-settings.json` immediately upon action. This ensures approval status survives Runner restart — previously, in-memory-only approval state was lost on restart, requiring operators to re-approve scenarios after every Runner bounce.

**REQ-STRAT-013b**: The persistence format in `develop-settings.json` MUST include scenario ID, approval status, timestamp, and any edited content (for Edit actions). The `DevelopSettingsService` handles read/write with file-lock safety.
...
- **REQ-SPR-001a**: When `SinglePRMode` is enabled (`VirtualDevTeam:SinglePRMode=true`), the SE MUST create exactly **one** engineering task (T1) that covers ALL enhancements from the PM's Enhancement issues. The task title and description SHALL consolidate all enhancement scope into a single work item.
...
- **REQ-WS3-002a**: Integration test scenarios SHALL be defined as JSON fixture files located under `tests/VirtualDevTeam.Integration.Tests/Fixtures/`. Each fixture describes: initial state (issues, PRs, labels), scripted AI responses, expected outcomes (created artifacts, phase transitions, final states).
...
3. `dotnet test tests/VirtualDevTeam.Integration.Tests` discovers the new fixture automatically
...
### REQ-WS3-003: State Snapshot Capture & Restore

- **REQ-WS3-003a**: `tests/temp/capture-state.ps1` MUST capture a named snapshot of the full VDT runtime state needed for stage-specific repros: runner SQLite DBs, local-platform bare repo, shared clone, scratch dirs, agent workspaces, worktree root, target repo checkout, and `develop-settings.json`.
- **REQ-WS3-003b**: Restore scripts such as `tests/temp/BeforePR_Local/setup.ps1` MUST restore a captured snapshot by stopping the runner, cleaning current state, copying the snapshot back into place, rebuilding, and restarting the runner so a known pipeline stage can be reproduced deterministically.
...
## 49. Multi-Platform Support (GitHub, Azure DevOps & Local)
...
- **REQ-PLAT-001b**: Platform selection is config-driven via `DevPlatformConfig.Platform` enum (`GitHub` | `AzureDevOps` | `Local`). DI registration switches all 7 interfaces based on this setting.
- **REQ-PLAT-001c**: GitHub adapters wrap the existing `IGitHubService` with zero breaking changes to existing behavior.
- **REQ-PLAT-001d**: Agent workspace clone URLs MUST be platform-aware. Use `VirtualDevTeamConfig.GetGitCloneUrl()` which returns the correct authenticated or local origin URL for GitHub, ADO, or Local mode.
...
### REQ-PLAT-006: Local Dev Platform Provider

- **REQ-PLAT-006a**: `LocalDevPlatform` MUST implement `IPullRequestService`, `IWorkItemService`, `IReviewService`, `IBranchService`, and `IRepositoryContentService`, backed by SQLite metadata plus a local bare git repository so agents preserve the existing multi-PR, multi-review quality loop.
- **REQ-PLAT-006b**: Local mode MUST satisfy enterprise branch-policy constraints by ensuring agents never merge directly to the protected upstream branch; all PR, review, and merge activity occurs inside the local provider.
- **REQ-PLAT-006c**: Dashboard behavior in Local mode MUST remain equivalent to hosted platforms, including PR cards, review timelines, and approval gates.
- **REQ-PLAT-006d**: After local completion, the system MUST support submitting one clean final PR to the enterprise repository for human review.
- **REQ-PLAT-006e**: `develop-settings.json` MUST support `devPlatformKind: "Local"` to enable Local mode.
- **REQ-PLAT-006f**: `LocalBareRepoManager.MergeBranchAsync` MUST attempt an automatic rebase fallback when a local merge hits conflicts. Only if both the merge and rebase fallback fail should Local mode surface the merge conflict as a blocking error.
- **REQ-PLAT-006g**: `LocalPullRequestService.GetFileDiffsAsync` MUST populate `PlatformFileDiff.Patch` from the local bare repo on demand so Local-mode reviewers and custom auditors (for example `SecurityAuditor`) can perform real code-aware review against actual patch content.

### REQ-PLAT-003: Dashboard Platform Configuration

- **REQ-PLAT-003a**: Configuration page MUST provide a Dev Platform dropdown (GitHub / Azure DevOps / Local).
...
- **REQ-PLAT-003d**: `DashboardDataService.PlatformName` MUST return "Azure DevOps", "GitHub", or "Local" based on `IPlatformHostContext` type.
...
- **REQ-PLAT-004a**: ADO work item states are configurable via `AzureDevOps.StateMappings` dictionary mapping VirtualDevTeam states (`Open`, `InProgress`, `Blocked`, `Resolved`) to ADO process template states.
...
## 50. Develop Wizard Clarifying Questions

> **Status (2026-05-04):** Fully implemented. JSONL parsing bug fixed. Prompt externalized to template. "Ask More" iterative questions and "Regenerate" with confirmation dialog both complete.

### REQ-CLARIFY-001: Question Generation

- **REQ-CLARIFY-001a**: The Develop wizard MUST generate clarifying questions when the user advances to the Clarify step, based on the project description and tech stack context.
- **REQ-CLARIFY-001b**: Questions MUST be generated via `CopilotCliProcessManager.ExecutePromptAsync()` using the template at `prompts/wizard/clarifying-questions.md`.
- **REQ-CLARIFY-001c**: When `CopilotCli.JsonOutput` is `true`, the raw JSONL output MUST be parsed via `CliOutputParser.ParseJsonOutput()` before question extraction. Failure to do this results in zero questions parsed.
- **REQ-CLARIFY-001d**: `ParseNumberedQuestions()` MUST handle numbered lines (`1.`, `1)`, `1:`), bulleted lines (`- `, `• `, `* `), and strip bold markers (`**text**` → `text`).
- **REQ-CLARIFY-001e**: Hash-based caching MUST prevent re-generation if the description is unchanged AND answers already exist.

### REQ-CLARIFY-002: Answer Merging

- **REQ-CLARIFY-002a**: `DevelopSettingsService.MergeIntoConfig()` MUST append all answered Q&A pairs under a `## Clarifying Details` section in the project description.
- **REQ-CLARIFY-002b**: Unanswered questions (empty answer field) MUST be skipped during merge.

### REQ-CLARIFY-003: Prompt Externalization

- **REQ-CLARIFY-003a**: The clarifying questions prompt MUST live in `prompts/wizard/clarifying-questions.md` with YAML frontmatter declaring `{{description}}` and `{{techContext}}` template variables.
- **REQ-CLARIFY-003b**: If the template file is missing, the wizard MUST fall back to an inline hardcoded prompt and log a warning.

**Scenario: Clarifying Questions Happy Path**
```
1. User enters a 2-sentence project description
2. User clicks Next to advance to Clarify step
3. Wizard calls TryGenerateClarifyingQuestionsAsync()
4. Template loaded from prompts/wizard/clarifying-questions.md
5. Prompt sent via ExecutePromptAsync(), JSONL output parsed
6. 8-10 numbered questions displayed in text fields
7. User answers questions and clicks Next
8. MergeIntoConfig() appends Q&A under "## Clarifying Details" in description
9. Full description (original + clarifying details) flows to PM agent
```

### REQ-CLARIFY-004: Iterative "Ask More" Questions

- **REQ-CLARIFY-004a**: The wizard MUST provide an "Ask More Questions" button after initial questions are generated.
- **REQ-CLARIFY-004b**: "Ask More" MUST use a separate prompt template (`prompts/wizard/clarifying-questions-more.md`) that receives `{{existing_questions}}` context to avoid duplicates.
- **REQ-CLARIFY-004c**: New questions MUST be deduplicated against existing questions using normalized text comparison AND token-overlap detection (>70% overlap threshold).
- **REQ-CLARIFY-004d**: The iteration count MUST be tracked via `ClarifyingIterationCount` and displayed in the button label (e.g., "Ask More Questions (Round 2)").
- **REQ-CLARIFY-004e**: After the LLM returns new questions, a status message MUST show how many new questions were added (e.g., "Added 5 new questions (3 duplicates filtered)").

### REQ-CLARIFY-005: Regenerate with Confirmation

- **REQ-CLARIFY-005a**: The wizard MUST provide a "Regenerate" button alongside "Ask More Questions".
- **REQ-CLARIFY-005b**: Regenerate MUST show a browser `confirm()` dialog warning the user that ALL existing questions and answers will be lost, with counts (e.g., "This will remove all 20 existing questions (including 15 with answers)").
- **REQ-CLARIFY-005c**: If the user cancels the confirm dialog, NO changes MUST occur — questions, answers, and iteration state remain intact.
- **REQ-CLARIFY-005d**: If confirmed, Regenerate MUST clear all answers, reset `ClarifyingIterationCount` to 0, clear the source hash, and generate a fresh batch of questions.

**Scenario: Ask More and Regenerate**
```
1. User generates initial 10 clarifying questions
2. User answers 7 of 10 questions
3. User clicks "Ask More Questions"
4. Wizard generates 10 more questions via clarifying-questions-more.md
5. Dedup removes 3 overlapping questions, 7 new questions appended (total: 17)
6. Status shows "Added 7 new questions (3 duplicates filtered)"
7. User decides to start fresh, clicks Regenerate
8. Confirm dialog: "This will remove all 17 existing questions (including 7 with answers). Are you sure?"
9. User confirms — all questions cleared, fresh batch of 10 generated
```

---

## 52. Pre-Publish Self-Assessment Requirements

> **Status (2026-05-04):** Implemented. Engineer agents self-assess PRs before marking ready-for-review. Implementation handoff context preserves decisions from the coding phase.

### REQ-ASSESS-001: Assessment Gate

- **REQ-ASSESS-001a**: Before any engineer agent marks a PR as `ready-for-review`, it MUST run `RunPrePublishAssessmentAsync()` to evaluate the implementation against the original issue requirements.
- **REQ-ASSESS-001b**: Self-assessment MUST use a FRESH `ChatHistory` (not the implementation conversation) to avoid self-justification bias.
- **REQ-ASSESS-001c**: The assessment reads: (1) the original issue title and body, (2) the list of changed files via `GetDiffFileListVsMainAsync()`, and (3) workspace file contents for each changed file.
- **REQ-ASSESS-001d**: The assessment prompt templates live in `prompts/engineer-base/self-assessment-system.md` and `prompts/engineer-base/self-assessment-user.md`.

### REQ-ASSESS-002: JSON Verdict

- **REQ-ASSESS-002a**: The LLM MUST return a JSON verdict with `verdict` (PASS or NEEDS_CHANGES), `confidence` (0.0–1.0), `summary`, and `gaps` (array of specific missing items).
- **REQ-ASSESS-002b**: `ParseAssessmentVerdict()` MUST extract JSON from fenced code blocks or raw JSON, falling back to PASS on parse failure to avoid blocking the pipeline.

### REQ-ASSESS-003: Automated Fix Attempts

- **REQ-ASSESS-003a**: On NEEDS_CHANGES verdict, the agent MUST attempt a surgical fix via `AttemptSelfAssessmentFixAsync()` using the `self-assessment-fix-system.md` prompt.
- **REQ-ASSESS-003b**: The fix prompt receives the specific `gaps` from the assessment and the workspace context.
- **REQ-ASSESS-003c**: After fix, the agent MUST rebuild and re-assess. Maximum 2 fix attempts before proceeding anyway with a warning log.

### REQ-ASSESS-004: Implementation Handoff Context

- **REQ-ASSESS-004a**: During implementation, engineer agents MUST record key decisions via `RecordImplementationNote()` — plan generation choices, build failure resolutions, scope adjustments.
- **REQ-ASSESS-004b**: `BuildImplementationHandoffContext()` MUST assemble these notes (capped at 20) plus recent activity log (10 entries from `StateStore`) into a `{{implementation_context}}` template variable.
- **REQ-ASSESS-004c**: The handoff context MUST be included in the assessment prompt so the fresh AI context understands WHY certain implementation choices were made.

### REQ-ASSESS-005: Integration Points

- **REQ-ASSESS-005a**: Assessment MUST be called from `EngineerAgentBase.MarkPrCompleteAsync()` and `CommitAndNotifyAsync()` for base-class completion paths.
- **REQ-ASSESS-005b**: Assessment MUST ALSO be called from `SoftwareEngineerAgent.FinalizeReadyForReviewAsync()` which bypasses base class completion paths.
- **REQ-ASSESS-005c**: `SpecialistEngineerAgent` inherits coverage via `EngineerAgentBase` and does NOT need separate wiring.

### REQ-ASSESS-006: Pre-Publish Screenshot Expectation Check (2026-05-11)

- **REQ-ASSESS-006a**: Before the self-assessment LLM runs, the engineer MUST run a pre-publish screenshot check via `RunPrePublishScreenshotCheckAsync(pr, ct)`. The method captures a screenshot of the running app and asks vision-AI whether what's rendered matches what the PR's title/body promised to deliver.
- **REQ-ASSESS-006b**: The vision check MUST use `PullRequestWorkflow.EvaluateScreenshotAgainstExpectationsAsync` and return a structured verdict (`MATCHES` / `DOES_NOT_MATCH` / `INCONCLUSIVE`) with confidence (0.0–1.0).
- **REQ-ASSESS-006c**: When verdict is `DOES_NOT_MATCH` with confidence ≥ 0.6, the screenshot check MUST surface the verdict as an implementation note (`RecordImplementationNote`) so the self-assessment LLM folds it into its gap analysis. The self-assessment prompt (`engineer-base/self-assessment-system.md`) MUST treat such a note as a HARD GAP requiring rework.
- **REQ-ASSESS-006d**: When verdict is `INCONCLUSIVE` or `MATCHES`, the note is informational only — the run continues normally. Backend-only PRs MUST NOT be false-flagged (the vision prompt instructs the AI to return `matches:true, confidence:0.0` when the PR has no UI deliverable).
- **REQ-ASSESS-006e**: The check is wired into BOTH `EngineerAgentBase.MarkPrCompleteAsync` (specialists) AND `SoftwareEngineerAgent.FinalizeReadyForReviewAsync` (SE's bypass path). Cross-cutting features MUST hit both paths per the Lesson #14/#16 rule.
- **REQ-ASSESS-006f**: The check is safe-by-default — any failure (no workspace, no vision model, capture exception) is a no-op and does NOT block the PR.

### REQ-ASSESS-007: Idempotent-Startup Architect/Engineer Prompt Rules (2026-05-11)

- **REQ-ASSESS-007a**: The architect's data-model prompt template (`prompts/architect/multi-turn-data-model.md`) MUST require seed code to be idempotent: EF Core `OnModelCreating + HasData`, OR `INSERT OR IGNORE` / `INSERT ... ON CONFLICT DO NOTHING`, OR check-then-insert.
- **REQ-ASSESS-007b**: The engineer's single-pass implementation prompt (`prompts/engineer-base/single-pass-implementation.md`) MUST instruct engineers to mentally run `dotnet run` / `npm start` TWICE before considering startup code complete. Second run must NOT crash on UNIQUE-constraint violations.
- **REQ-ASSESS-007c**: Forbidden pattern: `EnsureCreated()` combined with imperative INSERT into UNIQUE columns. The prompts MUST explicitly call this out as the most common silent-failure mode (backend crash ⇒ `/api/config/*` 500s ⇒ frontend renders blank canvas ⇒ pipeline approves a broken PR).

**Scenario: Self-Assessment with Fix**
```
1. Engineer finishes implementation, commits, pushes
2. RunPrePublishAssessmentAsync() fires with fresh ChatHistory
3. LLM reads issue requirements + changed files + implementation handoff notes
4. Verdict: NEEDS_CHANGES — gaps: ["Missing error handling for null input", "No unit test for edge case"]
5. AttemptSelfAssessmentFixAsync() runs with gap-specific prompt
6. Agent writes fixes, rebuilds (success)
7. Re-assessment: PASS — confidence 0.92
8. Agent proceeds to SyncBranchWithMainAsync → PRCodeComplete gate → MarkReadyForReview
```

**Scenario: Pre-Publish Screenshot Check Catches Blank Canvas**
```
1. Engineer finishes implementation, commits, pushes
2. RunPrePublishScreenshotCheckAsync captures a screenshot of the running app
3. PlaywrightRunner.CaptureAppScreenshotAsync detects blank canvas via ScreenshotQualityChecker
   (PNG < 15 KB on 1000+px canvas → uniform fill heuristic) → returns null
4. Screenshot check records: "Pre-publish screenshot check on PR #1438: no screenshot captured —
   if this PR was supposed to deliver a UI, the self-assessment must verify the app actually starts"
5. RunPrePublishAssessmentAsync sees this in implementation_context, asks LLM to verify
6. LLM checks `dotnet run` output → finds SQLite UNIQUE constraint crash → returns NEEDS_CHANGES
   with gap: "Backend crashes on startup: seed data uses imperative INSERT instead of HasData"
7. AttemptSelfAssessmentFixAsync patches seed code to use HasData()
8. Re-assessment: PASS — confidence 0.85
9. Agent proceeds to MarkReadyForReview
```

---

## 51. Complexity-Based PR Sizing

> **Status (2026-05-04):** Implemented. Complexity assessment + task normalization + CSS-with-feature rule active in SE agent.

### REQ-SIZING-001: Complexity Assessment

- **REQ-SIZING-001a**: Before engineering plan generation, `AssessProjectComplexity()` MUST classify the project as Small (≤3 issues AND ≤10 arch sections → max 3 tasks), Medium (≤7 issues AND ≤25 arch sections → max 6 tasks), or Large (8+ issues → max 10 tasks).
- **REQ-SIZING-001b**: The complexity assessment MUST be injected into the plan generation prompt so the LLM is aware of the target task count.

### REQ-SIZING-002: Task Normalization

- **REQ-SIZING-002a**: `NormalizeTaskPlan()` MUST merge the smallest tasks into nearest siblings when the task count exceeds the complexity-based cap.
- **REQ-SIZING-002b**: Merge target priority: same wave + same parent > same wave > shared dependency > any adjacent task.
- **REQ-SIZING-002c**: Normalization MUST run BEFORE issue creation to prevent orphan work items.

### REQ-SIZING-003: CSS-with-Feature Rule

- **REQ-SIZING-003a**: CSS/styling-only tasks MUST be merged with their feature task, never standalone. This prevents orphan CSS PRs that break styling when applied out of order.

### REQ-SIZING-004: Dependency Guard

- **REQ-SIZING-004a**: `AreDependencyPrsMerged()` MUST check `ListMergedAsync()` at task assignment time. Tasks with unmerged dependencies MUST be skipped even if the agent is idle.

---

## 53. Pre-PR Clarification Questions

> **Status (2026-05-06):** Implemented. Engineers generate clarifying questions before PR implementation, with configurable human gate for answer approval.

### REQ-CLARIFY-001: Question Generation

- **REQ-CLARIFY-001a**: Before beginning implementation on any task, engineer agents MUST call `GeneratePrePRQuestionsAsync()` to generate up to 10 clarifying questions about the task requirements.
- **REQ-CLARIFY-001b**: Questions MUST include AI-proposed answers that reflect the agent's interpretation of the requirements, allowing humans to confirm or correct assumptions.
- **REQ-CLARIFY-001c**: Each question MUST include `impactLevel` (xsmall, small, medium, large, xlarge) and `category` (architecture, ui, data, security, testing, integration, scope, other).
- **REQ-CLARIFY-001d**: The prompt template (`prompts/engineer-base/pre-pr-questions.md`) receives issue context, PMSpec, Architecture doc, and tech stack as input.

### REQ-CLARIFY-002: Human Gate Integration

- **REQ-CLARIFY-002a**: When `PrePRClarificationGate` is enabled (default: true), the agent MUST pause and wait for human approval before proceeding with implementation.
- **REQ-CLARIFY-002b**: Questions appear on the Approvals page with editable answer textareas pre-filled with proposed answers.
- **REQ-CLARIFY-002c**: Humans can edit answers, then approve — edited answers replace proposed answers as the final implementation guidance.
- **REQ-CLARIFY-002d**: On rejection, the agent MUST proceed with the original proposed answers (graceful degradation, not blocking).
- **REQ-CLARIFY-002e**: When the gate is disabled, `AutoApprove(id)` immediately uses proposed answers without waiting.

### REQ-CLARIFY-003: Decision Logging

- **REQ-CLARIFY-003a**: Each question-answer pair MUST be logged as an `AgentDecision` with impact level classification.
- **REQ-CLARIFY-003b**: Decisions are visible on the Reasoning page for post-hoc review.
- **REQ-CLARIFY-003c**: The `wasAutoApproved` flag distinguishes human-reviewed answers from auto-approved ones.

### REQ-CLARIFY-004: Dual-Path Wiring

- **REQ-CLARIFY-004a**: Clarification MUST be called from `EngineerAgentBase.WorkOnIssueAsync()` (used by SpecialistEngineerAgent).
- **REQ-CLARIFY-004b**: Clarification MUST ALSO be called from `SoftwareEngineerAgent.WorkOnOwnTasksAsync()` which bypasses the base class task pickup path.
- **REQ-CLARIFY-004c**: `SpecialistEngineerAgent` inherits coverage via `EngineerAgentBase` and does NOT need separate wiring.

### REQ-CLARIFY-005: Configuration

- **REQ-CLARIFY-005a**: The `PrePRClarificationGate` setting in `develop-settings.json` controls whether human approval is required (default: true).
- **REQ-CLARIFY-005b**: The Develop wizard MUST include a toggle for this setting on the first page ("Require Approval" vs "Auto-proceed").
- **REQ-CLARIFY-005c**: When enabling the gate, the master `HumanInteraction.Enabled` flag MUST be auto-set to true to prevent silent no-op.
- **REQ-CLARIFY-005d**: `PrePRClarificationStore` MUST be registered as a singleton in both Runner (`Program.cs`) and Standalone Dashboard (`StandaloneServiceRegistration`).

### REQ-CLARIFY-006: Context Injection

- **REQ-CLARIFY-006a**: Finalized question/answer context MUST be injected into the implementation planning prompt so the LLM uses the approved answers.
- **REQ-CLARIFY-006b**: In the SE direct path, clarification context MUST be appended to the PR description under an "## Implementation Decisions" heading.

**Scenario: Pre-PR Clarification with Human Review**
```
1. Engineer picks up task "Implement user authentication"
2. GeneratePrePRQuestionsAsync() generates questions:
   - "Should auth use JWT or session cookies?" → Proposed: "JWT based on Architecture.md spec"
   - "Should password reset be in scope?" → Proposed: "No, not in issue acceptance criteria"
3. Gate enabled → questions appear on Approvals page with bell notification
4. Human edits answer 1 to "Use session cookies for SSR app" → approves
5. Agent receives finalized answers, incorporates into implementation plan
6. Each Q&A logged as AgentDecision (medium impact, architecture category)
```


---

## Section: Image Generation Requirements

The Artist agent + any other role producing visual assets calls Azure OpenAI image generation REST endpoints directly (no MCP wrapper). Configuration lives in `develop-settings.json` under `AzureOpenAIImage`. The shared recipe is `prompts/_shared/image-gen-instructions.md`. Implementation: `src/VirtualDevTeam.Core/AI/ImageGenerationService.cs`, `AzureImageAuthProvider.cs`. Live artifact streaming to dashboard: `src/VirtualDevTeam.Core/Frameworks/CandidateArtifactWatcher.cs`.

### REQ-IMG-001: Deployment ladder requires MULTIPLE deployments

**You must provision multiple gpt-image-* deployments in your Azure OpenAI resource for the deployment ladder fallback to work.** The ladder walks deployments in the configured order on transient failures (429 rate-limit, 503 capacity, 404 deployment-not-found). With only one deployment configured, a single rate-limit storm halts every image-gen task in the run.

- **REQ-IMG-001a**: The recommended ladder is `gpt-image-1.5 → gpt-image-1 → gpt-image-1-mini → gpt-image-2`. Configure via `AzureOpenAIImage.PrimaryDeployment` + `AzureOpenAIImage.FallbackDeployments` in `develop-settings.json`. Each entry must be a real deployment name that exists in the Azure resource. Operator-validated 2026-05-12: gpt-image-1.5 produces dramatically more visual detail (full warriors/ornate towers) at the same prompt that yields simpler outputs from gpt-image-1 + gpt-image-1-mini, and gpt-image-2 has the tightest quota (2 RPM) so it sits at the bottom as a last-resort fallback.
- **REQ-IMG-001b**: ALL deployments in the ladder MUST be deployed in the same Azure OpenAI resource. They share the resource's auth (`api-key` or DefaultAzureCredential bearer token).
- **REQ-IMG-001c**: When only ONE deployment is provisioned, the ladder reduces to single-shot retries — the system functions but loses resilience. The Develop wizard's "Validate Image Gen" button probes only the primary; configure your fallbacks to deployments you've also provisioned.
- **REQ-IMG-001d**: **Within an animation cycle, every frame MUST be generated by the same model.** The deployment ladder kicks in PER CALL — if frame 0 of a goblin walk used gpt-image-1 successfully, frame 1 of the SAME walk should also use gpt-image-1. Do NOT switch models mid-animation; the visual character drifts (gpt-image-2 vs mini have noticeably different art styles). Cross-entity ladder choices are independent (cannon-tower can use gpt-image-1 while orc uses gpt-image-1-mini if the rate limit hit between them).

### REQ-IMG-002: Auth method must be set per-Azure-resource

- **REQ-IMG-002a**: `AzureOpenAIImage.AuthMethod = "DefaultAzureCredential"` (preferred — uses Entra/Managed Identity/Azure CLI; no key management). The runner snapshots a fresh bearer token (~1h TTL) into `AZURE_OPENAI_IMAGE_BEARER` for each child process.
- **REQ-IMG-002b**: `AzureOpenAIImage.AuthMethod = "ApiKey"` reads the static API key from `dotnet user-secrets` under key `VirtualDevTeam:AzureOpenAI:ImageApiKey`. Injected as `AZURE_OPENAI_IMAGE_API_KEY` for child processes.
- **REQ-IMG-002c**: Azure OpenAI image-gen endpoints use the `api-key` HTTP header for **all** API key sizes — NOT `Bearer`. Recipes that switch on key length (e.g., "if key > 60 chars use Bearer") are wrong and produce 401 → silent System.Drawing fallback. The shared recipe in `prompts/_shared/image-gen-instructions.md` is the source of truth; agents must replace any committed helper script with conflicting auth logic.

### REQ-IMG-003: Foundry vs classic Azure OpenAI resource

- **REQ-IMG-003a**: Foundry-managed Azure OpenAI resources return HTTP 404 on data-plane GET endpoints (`/openai/deployments` LIST, `/openai/deployments/{id}` individual). Only POST to `/openai/deployments/{id}/images/generations` returns valid 200 + b64_json. The "Validate Image Gen" probe in the Develop wizard MUST POST a tiny smoke-test prompt; relying on GET to enumerate deployments will report "no deployments" even when the resource is fully provisioned.

### REQ-IMG-004: No fabrication policy

- **REQ-IMG-004a**: Agents MUST NEVER fabricate PNGs from any other source — Pillow primitives, ImageMagick rectangles, ASCII art, byte sequences, etc. Any local-generation path that produces "art" is forbidden as a fallback when the API call fails.
- **REQ-IMG-004b**: When every deployment in the ladder fails for a given asset, the file MUST be left ABSENT and the failure surfaced in the PR description. Absent assets are honest; fabricated PNGs are dishonestly indistinguishable from success at the gate but unusable in the application.
- **REQ-IMG-004c**: Pillow / ImageMagick / Sharp ARE allowed for **post-processing** of API outputs (resizing, atlas composition, format conversion). The "no fabrication" rule applies to art CONTENT GENERATION only; geometric transformations of already-generated frames are explicitly permitted.

### REQ-IMG-005: Verification MUST use PNG signature, not file size

- **REQ-IMG-005a**: Every saved PNG MUST be verified by checking the first 4 magic bytes (`89 50 4E 47`) — NOT by file size. Real outputs range from a few KB (small icons) to >2 MB (1024×1024 master frames); size says nothing about correctness, the signature is the universal truth check.

### REQ-IMG-006: Parallelization for multi-asset tasks

- **REQ-IMG-006a**: Tasks producing >3 image deliverables MUST batch image-gen calls in waves of max 8 concurrent. Each wave honors a dependency graph: master frames first (no deps, fully parallel across entities), then variant frames (depend on master, parallel within entity).
- **REQ-IMG-006b**: Wall-clock comparison: 32 sequential calls × 30s ≈ 16 min; 3 waves of ≤8 parallel ≈ 2.5 min (6× speedup). Never run multi-asset image-gen serially.

### REQ-IMG-007: Live artifact streaming to dashboard

- **REQ-IMG-007a**: While a candidate's framework session is running, `CandidateArtifactWatcher` polls the worktree every 5s for newly-created or modified files (PNG/JSON/code) and emits a `FrameworkActivityEvent` per file. The Strategy Framework dashboard page (`/strategies`) renders these as `🎨 path/to/image.png (size KB)` activity entries with timestamps, so operators see assets land in real time instead of waiting for the post-execution patch summary.
- **REQ-IMG-007b**: The watcher excludes scaffolding paths (`.git`, `.squad`, `.copilot`, `.sandbox`, `node_modules`, `bin`, `obj`) so only candidate-produced output is surfaced.

### REQ-IMG-008: Binary-quality gate dominates LLM judge ranking

- **REQ-IMG-008a**: When ≥2 candidates produce binary deliverables (PNGs/JPGs) AND their `CandidateBinaryQualityCheck` scores differ by ≥30 points (out of 100), the binary-quality gate dominates the LLM judge's text-based ranking. The judge cannot distinguish a 1.6 MB real gpt-image-1 PNG from a 60 KB Pillow stub by reading patch text alone.
- **REQ-IMG-008b**: Quality scoring is heuristic: PNGs ≥100 KB with valid signature count as "real"; PNGs <30 KB or with invalid signature count as "fake". Score = 100 × (real - fake) / total. Wired into `CandidateEvaluator`.

### REQ-IMG-009: PMSpec [image-deliverables] manifest is conditional

- **REQ-IMG-009a**: The PM agent emits a structured `[image-deliverables]` YAML block in PMSpec.md ONLY when the project description requires visual assets (sprite sheets, character art, UI icons, hero illustrations, logos, mockups, etc.). Backend-only / CLI / library projects MUST NOT have an empty manifest — emitting one would force a no-op Artist run.
- **REQ-IMG-009b**: The manifest structure follows project intent, NOT a hardcoded template: a game project gets a `sprites:` block; a CRM gets `icons:`; a marketing site gets `illustrations:`. The PM prompt enforces the generality rule (no project-specific keyword whitelists).



---

## Section: FlowMonitor Scenarios

The FlowMonitor is an always-on background watchdog. Spec at `prompts/flow-monitor/role-description.md`. Implementation in `src/VirtualDevTeam.Core/HealthMonitor/` + `src/VirtualDevTeam.Orchestrator/FlowMonitorService.cs`. Audit trail in SQLite `flow_findings` + `flow_actions` tables. UI on `/health` page (Flow Monitor card).

### REQ-FLOW-001: Hard rules

- **REQ-FLOW-001a**: FlowMonitor MUST never restart any process
- **REQ-FLOW-001b**: FlowMonitor MUST never recompile code
- **REQ-FLOW-001c**: FlowMonitor MUST never force-merge a pull request
- **REQ-FLOW-001d**: FlowMonitor MUST never modify source code in any branch
- **REQ-FLOW-001e**: FlowMonitor MUST never delete issues, PRs, or branches
- **REQ-FLOW-001f**: FlowMonitor MUST rate-limit actions to `MaxActionsPerHour` per rolling hour (default 12)
- **REQ-FLOW-001g**: FlowMonitor MUST deduplicate findings by `DedupKey` within `DedupWindowMinutes` (default 15)

### REQ-FLOW-002: Persistence + observability

- **REQ-FLOW-002a**: Every detection MUST be persisted as a row in `flow_findings`
- **REQ-FLOW-002b**: Every action attempt MUST be persisted as a row in `flow_actions`
- **REQ-FLOW-002c**: Each tick MUST be logged as a row in `flow_monitor_ticks` so the dashboard can show liveness
- **REQ-FLOW-002d**: The Health Monitor page MUST show a "Flow Monitor" card with last-tick pulse + recent findings + recent actions
- **REQ-FLOW-002e**: Each successful action MUST emit a notification via `GateNotificationService.AddNotificationAsync` so the bell icon surfaces it

### REQ-FLOW-003: Detection scenarios

#### Scenario FM-1: Stuck agent (Working >30min with no status update)

```
1. Agent X transitions to Working at 10:00
2. Agent X status remains "Working" with same StatusReason at 10:31
3. AgentStuckDetector emits Critical FlowFinding with DedupKey "agent-stuck:X"
4. KickAgentPollAction publishes a FlowMonitorNudge bus message to X
5. Action result Success → flow_actions row inserted, finding state → ActedOn
6. Dashboard bell icon shows new notification
```

#### Scenario FM-2: Phase=Completion but agents still Working

```
1. Workflow advances to Completion phase
2. PM agent is still Working (running its post-completion enhancement-review loop)
3. PhaseCompletionMismatchDetector emits Warning FlowFinding
4. KickAgentPollAction nudges PM
5. PM either picks up the nudge and goes Idle, or the next tick finds same condition
6. Within DedupWindowMinutes, repeat findings are suppressed
```

#### Scenario FM-3: Action rate limit reached

```
1. 12 actions have been taken in the last hour
2. New finding arrives → InsertFinding succeeds
3. Rate-limit check fails → action is NOT executed
4. flow_actions has no new row, finding state stays Open
5. Operator sees the queue building up on the Health page
6. Operator can investigate via reasoning log + raise the cap if appropriate
```

#### Scenario FM-4: Detector throws (defensive isolation)

```
1. A detector hits an unexpected exception (e.g., NullReferenceException)
2. FlowMonitorService catches it, logs at Warning level
3. Tick continues to other detectors
4. Failed detector's findings for that tick are dropped
5. flow_monitor_ticks row still inserted (with the detector NOT listed in the detectors column)
```

#### Scenario FM-5: Notification surfacing

```
1. Action runs successfully
2. EmitNotification builds a context string: "FlowMonitor took action `kick-agent-poll` on `agent-X`. Reason: ..."
3. GateNotificationService.AddNotificationAsync(gateId="flow-monitor:kick-agent-poll", ...) is called
4. Notification bell badge increments
5. Operator clicks bell → sees the FlowMonitor entry with full context
```

#### Scenario FM-6: Service is OFF

```
1. FlowMonitorConfig.Enabled = false
2. Service still ticks every PollIntervalSeconds
3. Each tick: skips detector loop, doesn't insert findings or actions
4. flow_monitor_ticks STILL inserted (with empty detectors column) so liveness pulse stays current
5. Dashboard shows "Flow Monitor — OFF" badge
```

#### Scenario FM-7: Per-detector / per-action toggle

```
1. Operator disables AgentStuckDetector via FlowMonitorConfig.Detectors["agent-stuck"] = false
2. Service still runs PhaseCompletionMismatchDetector
3. AgentStuckDetector is silent for this run
4. Re-enabling does not require restart — IOptionsMonitor picks up changes
```

#### Scenario FM-8: Platform parity

```
1. FlowMonitor uses the same DetectorContext on both GitHub and ADO targets
2. Detectors compare agent state, status reason, phase strings — all platform-agnostic
3. Actions go through IMessageBus or IPullRequestService — platform abstraction layer
4. No FlowMonitor code path uses Octokit, gh CLI, or ADO REST APIs directly
```

#### Scenario FM-9: Cold restart

```
1. Runner is restarted while FlowMonitor has open findings
2. flow_findings rows persist in SQLite
3. New runner instance starts FlowMonitorService
4. Service does NOT re-emit findings for already-detected conditions if their DedupKey is still within window
5. New conditions detected after restart create new findings as usual
```

#### Scenario FM-10: AI unavailability

```
1. Future "rubber-duck" detectors will optionally call an LLM to recommend an action
2. If the LLM is unreachable (CLI down, rate-limited), detector falls back to its static catalog
3. Detector emits the same finding it would have without AI; only the action recommendation step is skipped
4. The vetted action catalog handles the finding via the standard CanHandle / ExecuteAsync flow
5. No FlowMonitor function depends critically on AI being available
```

### REQ-FLOW-004: Configuration

- **REQ-FLOW-004a**: FlowMonitorConfig is bound from `VirtualDevTeam:FlowMonitor` section in `appsettings.json`
- **REQ-FLOW-004b**: Default config is `Enabled=true`, `PollIntervalSeconds=30`, `MaxActionsPerHour=12`, `DedupWindowMinutes=15`
- **REQ-FLOW-004c**: Per-detector enable map and per-action enable map are dictionaries keyed by id — missing entries default to enabled

### REQ-FLOW-005: Escalation Ladder (T1.2)

A finding is no longer one-action-and-done. Each repeated occurrence on the same `DedupKey` advances through three rungs:

- **REQ-FLOW-005a**: Rung 1 (attempt 1) — bus nudge via `KickAgentPollAction` (publishes `FlowMonitorNudge` to target agent)
- **REQ-FLOW-005b**: Rung 2 (attempt 2) — `PostExplicitAskAction` posts a structured comment to the agent's open PR (preferred) or work item naming the agent and the ask
- **REQ-FLOW-005c**: Rung 3 (attempt 3) — `EscalateToHumanAction` applies the `agent-stuck` label AND emits a non-auto-resolving `GateNotificationService` notification
- **REQ-FLOW-005d**: Rung selection is by `FlowMonitorPersistence.GetAttemptCount(dedupKey, lookback=4h)`. Falls back to first `CanHandle` action if a rung-specific action isn't registered (graceful degradation)
- **REQ-FLOW-005e**: `flow_actions.attempt_count` column persisted; visible in the Health Monitor flow card timeline
- **REQ-FLOW-005f**: Rate limit (`MaxActionsPerHour`) still applies to the GLOBAL action count, not per-rung. Ladder just sequences the budget

### REQ-FLOW-006: Verification-After-Action Loop (T1.3)

Findings no longer sit in `ActedOn` indefinitely:

- **REQ-FLOW-006a**: At the start of each tick, FlowMonitor calls `VerifyActedOnFindingsAsync` BEFORE the regular detector loop
- **REQ-FLOW-006b**: For each finding still `ActedOn` and detected within last 1h, the originating `IFlowDetector.DetectAsync` is invoked again. If no matching finding for the same target re-emerges, the original is marked `Resolved`
- **REQ-FLOW-006c**: If the condition persists, the finding's severity is bumped (Info→Warning→Critical) and the finding is marked `Expired` so the dedup window doesn't suppress fresh re-emission, which then escalates via the next rung
- **REQ-FLOW-006d**: Verification attempts are persisted as their own `flow_action` row with `ActionType="verify-acted-on"`, `Result=Success` (cleared) or `Failed` (persists). Visible in the dashboard timeline
- **REQ-FLOW-006e**: Resolved findings emit a `FlowMonitorEvent` (Kind=ActionResult, "✅ Resolved: …") visible in the live log stream

### REQ-FLOW-007: Real-Time Log Streaming (T1.4)

A live, Copilot-CLI-styled event stream from FlowMonitor to a browser terminal:

- **REQ-FLOW-007a**: `FlowMonitorEventBus` is a singleton holding a bounded `Channel<FlowMonitorEvent>(capacity:200, DropOldest)` so the runner thread never blocks on backpressure
- **REQ-FLOW-007b**: FlowMonitorService publishes events on tick start, per-detector start/finish (with finding count), each finding (including dedup-suppressed and rate-limited), each action started/completed, service errors
- **REQ-FLOW-007c**: Each event is auto-tagged with `AgentCallContext.CurrentAgentId/SessionId` (AsyncLocal flow)
- **REQ-FLOW-007d**: SignalR hub at `/hubs/flowmonitor` fans the bus out to subscribed circuits via `FlowMonitorEventRelay : BackgroundService`
- **REQ-FLOW-007e**: Dashboard page `/flow-monitor-log` renders the stream in an xterm.js terminal with Copilot-CLI color classification (purple=finding, green=success, red=error/failed, cyan=detector, gray=lifecycle)
- **REQ-FLOW-007f**: Verbosity selector LOW / MEDIUM / HIGH filters client-side — server broadcasts unconditionally so toggling is instant

### REQ-FLOW-008: FixRecommendation Pipeline (T1.5)

Critical findings without an action handler trigger an AI-generated fix recommendation:

- **REQ-FLOW-008a**: `FixRecommendationPlannerService` runs a 2-pass Copilot CLI flow: pass 1 = `/plan` produces a structured fix plan; pass 2 = rubber-duck critique of that plan only (no original finding context, prevents anchoring)
- **REQ-FLOW-008b**: Recommendations are persisted to a new SQLite table `fix_recommendations` AND saved as `.md` files under `/FixRecommendations/{yyyyMMdd-HHmmss}-{finding-id-short}.md`
- **REQ-FLOW-008c**: Reworks (Approvals page → "Rework") insert a NEW row (history preserved) with `rework_count++`. Plan files use `-rework-v{N}` suffix; never overwritten
- **REQ-FLOW-008d**: A recommendation with `Confidence ≥ 0.75` (configurable) emits a notification routed to the Approvals page; below-threshold ones persist silently
- **REQ-FLOW-008e**: Approvals page renders the plan via Markdig + Ganss.Xss sanitization with severity, confidence, files-touched, and restart-required badges. Operator can Approve, Rework (edit plan), or Reject
- **REQ-FLOW-008f**: All operations exposed via REST endpoints under `/api/dashboard/health/flow-monitor/recommendations/{id}/`: `approve`, `rework`, `reject`

### REQ-FLOW-009: Code-Fix-Without-Restart Classifier (T1.6)

Approved recommendations route through one of three apply paths based on what the plan touches:

- **REQ-FLOW-009a**: `FixClassifier.Classify(rec)` returns `FixTier { Live | DeferredRestart | Blocked }` based on file-pattern heuristics:
  - `Live` — only `prompts/**/*.md`, `appsettings.json` keys, `develop-settings.json`, `prompts/sme-templates/*.json`, or FlowMonitor-only config
  - `DeferredRestart` — any `.cs` or `.razor` file change
  - `Blocked` — any `.csproj`, `package.json`, NuGet add, or DB migration file
- **REQ-FLOW-009b**: `fix_tier` and `affected_files` columns added to `fix_recommendations` table (additive migration via `TryAddColumn`)
- **REQ-FLOW-009c**: Tier badge (🟢 Live / 🟡 Deferred / 🔴 Blocked) displayed in Approvals card AND in the Health Monitor expanded-finding panel
- **REQ-FLOW-009d**: Live tier — `LiveFixApplicator` invokes Copilot CLI agentic mode against repo root with constrained system prompt ("touch ONLY these files"), then verifies via `git status --porcelain` snapshot diff that no out-of-scope file was modified. Verified → state `Applied`. Scope violation → state `AppliedFailed`, diff saved to `/FixRecommendations/{id}-failed.diff`
- **REQ-FLOW-009e**: Deferred-Restart tier — same CLI invocation, final state `Coded`, notification reads "Fix coded — click Restart Runner on Health Monitor to activate"
- **REQ-FLOW-009f**: Blocked tier — no CLI run; state `StagedForNextRestart`, plan moved to `/FixRecommendations/staged/{id}.md`. `StagedFixApplicator : IHostedService` runs at startup BEFORE FlowMonitorService, applies each staged fix via Copilot CLI, runs `dotnet build`, moves applied to `/FixRecommendations/applied/`. Build failure blocks further startup so the operator sees the issue
- **REQ-FLOW-009g**: All CLI invocations go through the existing `CopilotCliProcessManager` — never spawn `copilot.exe` directly

### REQ-FLOW-010: Warm Restart (T1.7)

Operator can restart the runner without losing workflow state:

- **REQ-FLOW-010a**: New endpoint `POST /api/dashboard/runtime/restart` spawns `scripts/restart-runner.ps1` as a detached helper, then calls `IHostApplicationLifetime.StopApplication()` after a brief flush delay
- **REQ-FLOW-010b**: `restart-runner.ps1` waits up to 30s for the runner PID to exit, force-kills if still alive, sleeps 2s for OS file-lock release, then re-launches via `start-runner.ps1`
- **REQ-FLOW-010c**: HealthMonitor.razor exposes a "🔄 Restart Runner" button on the Flow Monitor card with two-click confirm pattern (4s auto-revert)
- **REQ-FLOW-010d**: Workflow phase, agent identities, signals, CLI session IDs, and rework counts are checkpointed to SQLite and restored on next boot — durable platform work (PRs, issues, branches) is unaffected
- **REQ-FLOW-010e**: The auto-reload-on-disconnect logic in `App.razor` triggers a hard reload when the new runner is up, so open browser tabs recover automatically

### REQ-FLOW-011: Liveness Watchdog (T1.8)

The HealthMonitor watches FlowMonitor's heartbeat:

- **REQ-FLOW-011a**: At end of each `HealthMonitor.CheckHealth` cycle, `CheckFlowMonitorLiveness()` checks the latest `flow_monitor_ticks` row's `recorded_at`
- **REQ-FLOW-011b**: If staleness exceeds `2 × FlowMonitorConfig.PollIntervalSeconds`, fires `flow-monitor:liveness` notification ONCE (guarded by `_flowMonitorLivenessAlertSent` flag); does NOT auto-resolve
- **REQ-FLOW-011c**: When tick freshness recovers, the notification is resolved and the flag cleared so a future outage re-alerts
- **REQ-FLOW-011d**: Constructor accepts the new dependencies as optional `= null` defaults so tests and standalone Dashboard host are unaffected

### REQ-FLOW-012: Deadlock Detector Wrap (T2.13)

The existing `Orchestrator.DeadlockDetector` (wait-for graph DFS) is exposed as an `IFlowDetector` without modification:

- **REQ-FLOW-012a**: New `DeadlockFlowDetector` subscribes to `DeadlockDetector.DeadlockDetected` event in its constructor and caches events with a 5-minute TTL under a lock
- **REQ-FLOW-012b**: Replays cached events as `FlowFinding` records with `Severity=Critical`, `DedupKey=deadlock:{cycleHash}` where `cycleHash` is sorted-distinct agent ids (so A→B→C→A and B→C→A→B collapse to the same key)
- **REQ-FLOW-012c**: `DeadlockDetector` itself is unmodified — pure observer pattern
- **REQ-FLOW-012d**: Located in `src/VirtualDevTeam.Orchestrator/DeadlockFlowDetector.cs` (Core would create circular project dep on Orchestrator)
- **REQ-FLOW-012e**: Registered in `Program.cs` alongside other `IFlowDetector` registrations; appears in HealthMonitor.razor's `_knownDetectorIds` so the toggle UI shows up before any findings arrive

### REQ-FLOW-013: DetectorContext Platform Views (T1.1)

Detectors can inspect platform state (PRs, work items, reviews, commits) without injecting platform services directly:

- **REQ-FLOW-013a**: `DetectorContext.WorkflowSignals` is populated from `WorkflowStateMachine.GetSignals()` (was previously hardcoded `Array.Empty<string>()` — bug fix)
- **REQ-FLOW-013b**: `DetectorContext.Platform : IPlatformView` exposes 4 lazy methods: `ListOpenPullRequestsAsync`, `ListOpenWorkItemsAsync`, `ListUnresolvedThreadsAsync(prNumber)`, `GetLatestCommitAsync(prNumber)`
- **REQ-FLOW-013c**: `PerTickPlatformView` caches each fetched view for the lifetime of one DetectorContext (one tick) — multiple detectors sharing the same context only pay the API cost once
- **REQ-FLOW-013d**: All view methods are exception-safe: errors are logged at Warning and surfaced as empty results (the FlowMonitor must be MORE reliable than the system it watches)
- **REQ-FLOW-013e**: Detector-friendly view records (`PullRequestView`, `WorkItemView`, `ReviewThreadView`, `CommitView`) wrap the platform-neutral models with read-only fields and UTC-tagged `DateTimeOffset`
- **REQ-FLOW-013f**: `NullPlatformView.Instance` is used when platform services are unavailable (e.g., before a project is opened); every method returns an empty / null result so detectors never need null-checks on `ctx.Platform`

### REQ-FLOW-014: Smart Stuck Detection Thresholds

- **REQ-FLOW-014a**: `AgentStuckDetector` MUST recognize known long-running activities and apply an extended threshold (3× the default `StuckThresholdMinutes`) before flagging the agent as stuck. Known long-running activities include: strategy framework orchestration (`StatusReason` contains "strategy" or "framework"), self-assessment (`StatusReason` contains "self-assess"), rework cycles (`StatusReason` contains "rework"), and CLI edit mode (`StatusReason` contains "cli edit" or "CLI edit").
- **REQ-FLOW-014b**: The extended threshold prevents false-positive stuck alerts during legitimate multi-minute operations. Without this, the default 30-minute threshold fires during strategy runs (which routinely take 45–90 minutes for multi-candidate evaluation) and triggers unnecessary escalation ladder actions.
- **REQ-FLOW-014c**: The activity-recognition patterns MUST be configurable in `FlowMonitorConfig` so operators can add new long-running activity patterns without code changes. Default patterns are hardcoded as a fallback.
- **REQ-FLOW-014d**: When the extended threshold is applied, the finding's `Description` MUST note that the extended threshold was used (e.g., "Agent stuck for 95 min (extended threshold: 90 min for strategy framework)") so operators understand why the alert took longer to fire. *(Added 2026-05-15)*

### REQ-FLOW-020: Flow Monitor Dashboard

Replace the xterm.js terminal at `/flow-monitor-log` with a structured incident console at `/flow-monitor`:

- **REQ-FLOW-020a**: Primary view — **Active Issues** cards showing only Open/ActedOn findings, sorted by severity (Critical → High → Medium → Low). Each card is expandable to reveal full diagnostics (checklist, recommended fix, timeline).
- **REQ-FLOW-020b**: Secondary view — **Recent Changes** showing only status transitions (e.g., Open → ActedOn, ActedOn → Resolved). MUST suppress "0 finding(s)" tick noise — only meaningful state changes appear.
- **REQ-FLOW-020c**: Tertiary view — **All Detectors** grid showing registered detectors with last-run time and finding count. Collapsed by default; expandable on click.
- **REQ-FLOW-020d**: The raw xterm.js terminal MUST remain accessible at `/flow-monitor-log` as a debug/diagnostic link (e.g., footer link or nav sub-item). It is NOT removed — only demoted from the primary view.

### REQ-FLOW-021: Agent Disappearance Detector

FlowMonitor detector that fires when registered agent count drops unexpectedly during an active workflow phase:

- **REQ-FLOW-021a**: Fire a **Critical** finding when 0 agents are registered during any active phase (Initialization through Review). This catches `ResetCaches` bugs, silent agent crashes, and DI registration failures.
- **REQ-FLOW-021b**: Fire a **Warning** finding when the registered agent count drops below a minimum threshold (default: 3) during ParallelDevelopment or later phases. Threshold MUST be configurable in `FlowMonitorConfig`.
- **REQ-FLOW-021c**: Detector MUST NOT fire during Completion phase (agents are expected to wind down) or before any project is opened (pre-Initialization state).
- **REQ-FLOW-021d**: DedupKey format: `agent-disappearance:{phase}:{agentCount}` — collapses repeated alerts for the same count in the same phase.

### REQ-FLOW-022: Pipeline Stall Detector

- **REQ-FLOW-022a**: `PipelineStallDetector` MUST run only during active engineering phases (`ParallelDevelopment`, `Testing`, `Review`).
- **REQ-FLOW-022b**: It MUST emit a **Critical** finding for each open engineering task still labeled `status:blocked` when no open PR is actively working on that task. DedupKey format: `pipeline-stall:blocked:{issueNumber}`.
- **REQ-FLOW-022c**: It MUST emit a **Critical** finding when all engineer-role agents have been idle beyond the detector threshold, no open PRs exist, and claimable engineering work items still remain. DedupKey: `pipeline-stall:all-idle`.
- **REQ-FLOW-022d**: If all remaining engineering tasks are blocked and no stale-blocked per-task finding already explains the condition, the detector SHOULD emit a pipeline-level **Warning** with DedupKey `pipeline-stall:all-blocked`.

### REQ-FLOW-023: Diagnostic Action Executor Boundary

- **REQ-FLOW-023a**: FlowMonitor recommendation approve/reject API endpoints MUST delegate execution to `IDiagnosticActionExecutor` so the minimal API surface remains thin and the allowlisted apply/dismiss behavior is centralized.
- **REQ-FLOW-023b**: Recommendation rework MUST remain planner-owned because it creates a new recommendation row and preserves revision history rather than mutating the existing record in place.

### REQ-STRAT-030: Strategy Recovery Checkpoints

Persist candidate patches and execution metadata to SQLite after all candidates complete execution, enabling restart recovery without re-executing strategies:

- **REQ-STRAT-030a**: After each candidate completes execution (build + test), write a checkpoint row to `strategy_checkpoints` table containing: candidate ID, strategy ID, patch (unified diff), build result, test result, scores, and the `baseSha` at execution time.
- **REQ-STRAT-030b**: On restart, `TryRecoverFromCheckpointAsync` checks if the current HEAD matches the stored `baseSha`. If it matches, resume evaluation (judging, visual scoring, winner selection) without re-executing any strategies.
- **REQ-STRAT-030c**: Config toggle: `StrategyFrameworkConfig.RecoverOrphanedCandidates` (default `true`). When `false`, always re-execute from scratch.
- **REQ-STRAT-030d**: Fail-safe: any exception during recovery MUST be caught, logged at Warning, and fall through to fresh execution. Recovery failures MUST NOT block the strategy pipeline.

### REQ-STRAT-031: Visual Score Participates in Winner Selection

- **REQ-STRAT-031a**: `ApplyVisualScoresAsync` MUST run BEFORE the winner sort/pick logic in the evaluation pipeline, not after. Visual scores must be populated on each candidate before the ranking comparison.
- **REQ-STRAT-031b**: `VisualsScore` acts as a tiebreaker when code quality scores (build, test, LLM judge) are equal. Tie-breaking order: (1) highest total code quality score, (2) highest `VisualsScore`, (3) `llm-rank` preference, (4) strategy priority.

### REQ-STRAT-032: CaptureMode for Lightweight Screenshots

- **REQ-STRAT-032a**: `CaptureAppInteractionAsync` MUST accept a `CaptureMode` parameter (`ScreenshotOnly` | `FullMedia`).
- **REQ-STRAT-032b**: `ScreenshotOnly` mode skips MCP exploration, video recording, and GIF conversion — takes only static screenshots of the specified URLs. This avoids the 2–3 minute overhead of full media capture.
- **REQ-STRAT-032c**: Ready-for-review screenshots (post-implementation, pre-publish) MUST use `ScreenshotOnly` mode. Full media capture is reserved for strategy evaluation and explicit test artifact generation.

### REQ-STRAT-033: UI Task Pre-Flight Gate

- **REQ-STRAT-033a**: `MediaCaptureGate.ShouldCapture()` inspects the task description and list of changed files before spawning any Playwright processes. Returns `false` for pure backend/library/infrastructure tasks with no UI component.
- **REQ-STRAT-033b**: Heuristics: skip capture if changed files are exclusively `.cs`, `.csproj`, `.json`, `.yml`, `.md` (no `.razor`, `.html`, `.css`, `.js`, `.ts`, `.tsx`, `.jsx`, `.vue`, `.svelte`) AND task description contains no UI-related keywords (e.g., "page", "component", "button", "form", "layout", "dashboard", "UI", "visual").
- **REQ-STRAT-033c**: Gate decision is logged as an `AgentDecision` so the Reasoning page shows why capture was skipped.

### REQ-STRAT-034: Candidate Preview Source Classification

- **REQ-STRAT-034a**: Strategy previews MUST distinguish between `CaptureUnavailable` (browser/tooling missing), `CaptureFailed` (app failed to boot or produced unusable capture), and `NoVisualContent` (legitimate backend-only or non-visual work).
- **REQ-STRAT-034b**: Dashboard strategy tiles MUST render different operator-facing messaging for those three states so expected empty previews are not confused with environment/setup failures.
- **REQ-STRAT-034c**: Preview classification MUST prevent `NoVisualContent` from masking actual capture failures.

### REQ-UI-040: Interactive Walkthrough Tour

- **REQ-UI-040a**: New `/walkthrough` page displaying 22 animated GIFs demonstrating dashboard features (agent cards, approvals, timeline, repository browser, configuration, metrics, etc.).
- **REQ-UI-040b**: The Welcome wizard's final step MUST include a link to `/walkthrough` (e.g., "Take a tour of the dashboard →").
- **REQ-UI-040c**: GIFs are stored under `wwwroot/walkthrough/` and lazy-loaded to avoid blocking page render.

### REQ-UI-041: Reasoning Page Default Tab

- **REQ-UI-041a**: The Reasoning page (`/reasoning`) MUST default to the **Decisions** tab when first loaded, not the Steps tab. Decisions provide higher-signal information for monitoring agent behavior.
- **REQ-UI-041b**: Tab selection state is NOT persisted across page navigations — each visit resets to Decisions.
...
- **REQ-UI-043a**: Connector arrows (`→`) MUST be rendered between ALL adjacent phase columns in the Timeline view, not just between wave-grouped phases. Every phase transition (Initialization → Research → Architecture → … → Completion) gets a visual connector.
- **REQ-UI-043b**: Arrows use muted styling consistent with the existing timeline theme. Active-phase arrow is highlighted.

### REQ-UI-044: Operator Change Requests from PR Detail

- **REQ-UI-044a**: PR detail views MUST expose an operator-driven change-request action that lets a human submit targeted implementation feedback without leaving the dashboard.
- **REQ-UI-044b**: Submitted feedback MUST be posted as a structured `**[Operator] CHANGES REQUESTED**` comment with the feedback body sanitized to escape HTML comment delimiters before storage/posting.
- **REQ-UI-044c**: Submitting operator feedback MUST publish a `ChangesRequestedMessage` so the assigned engineer can pick up the rework even if they already moved on to another task.
- **REQ-UI-044d**: Operator-only rework MUST preserve existing human approvals, MUST NOT consume the standard `MaxReworkCycles` budget, and MUST close the loop with an `**[Operator-Addressed]**` completion comment once implemented.
- **REQ-UI-044e**: Operator feedback text MUST be copied into engineer implementation notes so downstream self-assessment context continues to honor the human-requested change.

### REQ-UI-045: New Story Wizard

- **REQ-UI-045a**: Timeline `+` actions MUST be able to open a modal `NewStoryWizard` scoped to the selected wave.
- **REQ-UI-045b**: The wizard MUST provide a 3-step flow: (1) capture story title, description, acceptance criteria, complexity, and optional dependencies, (2) generate/edit clarifying questions, (3) review the assembled story before creation.
- **REQ-UI-045c**: Creating the story MUST keep the operator in context (modal closes, timeline refreshes) rather than forcing navigation to a separate planning page.

### REQ-CONFIG-010: Reasoning Level Validation

- **REQ-CONFIG-010a**: The Configuration page's model settings dropdown MUST display only reasoning levels (e.g., low, medium, high, none) that are supported by the currently selected model. Unsupported levels are hidden or greyed out.
- **REQ-CONFIG-010b**: If the user switches to a model that does not support the currently configured reasoning level, display an inline warning and suggest the closest supported level.

### REQ-CONFIG-011: Default Gate Preferences

- **REQ-CONFIG-011a**: When no `develop-settings.json` exists (fresh clone / first run), the Develop wizard MUST default the following gates to `true`: `PMSpecification`, `ArchitectureDesign`, `PrePRClarification`.
- **REQ-CONFIG-011b**: Default values are read from `appsettings.json` under `VirtualDevTeam:HumanInteraction:Gates` — the wizard does NOT hardcode defaults in C#.
- **REQ-CONFIG-011c**: Once `develop-settings.json` is created by the wizard, it becomes the source of truth — `appsettings.json` defaults are only used for initial population.
