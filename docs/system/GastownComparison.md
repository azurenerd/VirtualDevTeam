# VirtualDevTeam vs Gas Town: Multi-Agent Orchestration Comparison

> **Purpose:** Side-by-side comparison of two multi-agent AI coding systems — VirtualDevTeam (azurenerd/VirtualDevTeam) and Gas Town (gastownhall/gastown). Both coordinate multiple AI agents to build software, but with fundamentally different architectures and philosophies.
...
- **VirtualDevTeam**: SQLite `agent_state` table + GitHub Issues/PRs as durable state
- **Gas Town**: Git worktree "hooks" + Beads ledger for issue state

### 3. GitHub Integration
Both use GitHub as a durable collaboration layer:
- VirtualDevTeam: PRs for code, Issues for tasks, Comments for discussion, Labels for workflow state
...
- **VirtualDevTeam**: Configurable human gates per phase (PM Spec, Architecture, Engineering Plan, Final PR Approval)
...
- **VirtualDevTeam**: Blazor Server dashboard with agent cards, timeline, health monitor, engineering plan graph
...
- **VirtualDevTeam**: `DeadlockDetector` (DFS wait-for graph), `HealthMonitor`
...
- **VirtualDevTeam**: Engineer → PM clarification → Executive request Issues
...
- **VirtualDevTeam**: SE's `MergeTestedPRsAsync` with label-gated squash merge
...
**VirtualDevTeam** assigns agents to **fixed SDLC roles** that mirror a real software team:
...
**Implication:** VirtualDevTeam produces richer, more structured output (specs, architecture docs, code reviews with role-specific perspectives) but is less flexible. Gas Town can tackle any kind of work but requires the human (or Mayor) to define the breakdown.

### 2. Workflow: Phase-Gated Pipeline vs Task Queue

**VirtualDevTeam** follows a **strict linear pipeline** with gate conditions:
```
...
**Implication:** VirtualDevTeam guarantees a complete SDLC process (you always get specs before code) but can't skip phases. Gas Town is more flexible but relies on human/Mayor judgment for process discipline.

### 3. Communication: In-Process Bus vs Inter-Process Messaging

**VirtualDevTeam**: `InProcessMessageBus` via `System.Threading.Channels` (bounded capacity 1000)
...
**Implication:** VirtualDevTeam is faster (in-process) but more fragile (single process = single point of failure). Gas Town is more resilient (agents are independent processes) and supports richer communication patterns (mail + nudge + seance).

### 4. AI Runtime: Single Provider vs Multi-Runtime

**VirtualDevTeam**: Routes all AI calls through Copilot CLI via `CopilotCliChatCompletionService`:
...
**Implication:** Gas Town's multi-runtime support is a major differentiator — you can use the best AI for each task. VirtualDevTeam's single-provider approach is simpler but less flexible.

### 5. Scale: Team Simulation vs Swarm Coordination

**VirtualDevTeam** targets **7-10 agents** simulating a small development team:
...
**VirtualDevTeam**: SQLite database + GitHub API
...
**VirtualDevTeam**: Built-in multi-tier test execution:
...
**VirtualDevTeam**: Three-phase sequential review with gate labels:
...
## Where VirtualDevTeam Shines

### 1. **End-to-End SDLC Simulation**
VirtualDevTeam is the only system that produces a complete software development lifecycle — from research and spec writing through architecture, engineering planning, parallel development, testing with visual evidence, and multi-phase code review. Gas Town coordinates workers but doesn't enforce (or even suggest) a structured development process.
...
Supporting 10+ AI runtimes (Claude, Copilot, Codex, Cursor, Gemini, etc.) with per-task overrides means you can use the best tool for each job. VirtualDevTeam is locked to Copilot CLI with fallback to API keys.

### 2. **Scale (20-50+ Agents)**
Gas Town's architecture (independent processes, tmux sessions, persistent mail, git-backed state) is designed for much larger agent populations. VirtualDevTeam's in-process model tops out around 10 agents in a single .NET process.

### 3. **Resilience & Fault Isolation**
Each Gas Town agent runs as an independent process. One agent crashing doesn't affect others. VirtualDevTeam runs all agents in a single process — a crash kills everything (though SQLite + GitHub enable restart recovery).
...
The Bors-style bisecting merge queue is more sophisticated than VirtualDevTeam's label-gated merge. It batches MRs, runs verification, and isolates failures automatically. VirtualDevTeam relies on the SE to manage merges sequentially.

### 6. **Multi-Project Support**
Gas Town's Rig model manages multiple projects simultaneously with independent agent pools, hooks, and refineries. VirtualDevTeam processes one project at a time.

### 7. **Watchdog Hierarchy**
The three-tier Witness → Deacon → Dogs monitoring chain with automatic recovery (nudge stuck agents, handoff context) is more mature than VirtualDevTeam's `DeadlockDetector` + `HealthMonitor`.

### 8. **Federation (Wasteland)**
Cross-organization work sharing via DoltHub with portable reputation stamps is a forward-looking feature that has no equivalent in VirtualDevTeam.

### 9. **Plugin Architecture**
Gas Town has a formal plugin system (compactor-dog, stuck-agent-dog, quality-review, github-sheriff, etc.) for extensibility. VirtualDevTeam's functionality is compiled into the core assemblies.

### 10. **OpenTelemetry Integration**
Structured telemetry with OTLP export to VictoriaMetrics/VictoriaLogs provides production-grade observability. VirtualDevTeam uses ILogger<T> structured logging but has no OTEL integration.
...
```
VirtualDevTeam (Single Process)              Gas Town (Multi-Process)
┌────────────────────────────┐          ┌─────────────────────────────┐
...
**VirtualDevTeam** is a **vertical solution** — deeply opinionated about how software should be built, with a complete SDLC pipeline, role-based specialization, and rich AI-powered code review with test generation and visual evidence. Best for teams that want a turnkey "AI development team" simulation.
...
They solve the same fundamental problem (multi-agent AI coordination) but at different layers of abstraction. VirtualDevTeam could potentially use Gas Town as its underlying coordination layer, while Gas Town could adopt VirtualDevTeam's SDLC pipeline as a formula/molecule workflow.
