...
VirtualDevTeam deploys 7 specialized agent roles, each implemented as a class extending `AgentBase`. Every agent runs an independent async loop (`RunAgentLoopAsync`) and communicates through the message bus and the dev platform (GitHub or Azure DevOps, via `IPullRequestService`, `IWorkItemService`, etc.).
...
**Class:** `ProgramManagerAgent` · **File:** `VirtualDevTeam.Agents/ProgramManagerAgent.cs`
...
**Class:** `ResearcherAgent` · **File:** `VirtualDevTeam.Agents/ResearcherAgent.cs`
...
**Class:** `ArchitectAgent` · **File:** `VirtualDevTeam.Agents/ArchitectAgent.cs`
...
**Class:** `SoftwareEngineerAgent` · **File:** `VirtualDevTeam.Agents/SoftwareEngineerAgent.cs`
...
**Class:** `TestEngineerAgent` · **File:** `VirtualDevTeam.Agents/TestEngineerAgent.cs`
