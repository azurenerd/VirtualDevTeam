# VirtualDevTeam Workflow Monitoring Checklist
...
$pat = $settings.VirtualDevTeam.Project.GitHubToken
...
# Replace {owner/repo} with your actual repository (e.g., from develop-settings.json)
$repo = "owner/repo"
$headers = @{ Authorization = "token $token"; Accept = "application/vnd.github.v3+json" }

# Check PR labels
(Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/pulls/707" -Headers $headers).labels.name

# Check open PRs
(Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/pulls?state=open" -Headers $headers) | ForEach-Object { "$($_.number): $($_.title) [$($_.labels.name -join ',')]" }

# Check open issues
(Invoke-RestMethod -Uri "https://api.github.com/repos/$repo/issues?state=open&per_page=100" -Headers $headers) | Where-Object { -not $_.pull_request } | ForEach-Object { "$($_.number): $($_.title)" }
```
